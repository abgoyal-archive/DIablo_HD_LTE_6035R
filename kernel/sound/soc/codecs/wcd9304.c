...
struct sitar_priv {
	...
	u32 ldo_h_count;
        u32 micbias_enable_count[SITAR_NUM_MICBIAS];
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	micbias_enable_count = &sitar->micbias_enable_count[micb_line];
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (wreg == sitar->mbhc_bias_regs.ctl_reg) {
			SITAR_ACQUIRE_LOCK(sitar->codec_resource_lock);
			sitar_codec_switch_micbias(codec, 0);
			SITAR_RELEASE_LOCK(sitar->codec_resource_lock);
		}

/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2013/7/4, No need to reset the 4th bit of this reg here*/
#ifdef CONFIG_TCT_8X30_COMMON
		snd_soc_update_bits(codec, wreg, 0x0E, 0x00);
#else
		snd_soc_update_bits(codec, wreg, 0x1E, 0x00);
#endif
...
static const struct snd_soc_dapm_widget sitar_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MICBIAS_E("MIC BIAS2 Power External",
		SITAR_A_MICB_2_CTL, 7, 0,
		sitar_codec_enable_micbias_power,
		SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMU |
		SND_SOC_DAPM_POST_PMD),
...
void sitar_codec_calibrate_rel(struct snd_soc_codec *codec) {

	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
	if (sitar->mbhc_state == MBHC_STATE_POTENTIAL_RECOVERY) {
		pr_err("%s: MBHC being recovered, do not update release thresholds\n", __func__);
		return;
	} else {
		snd_soc_write(codec, SITAR_A_CDC_MBHC_VOLT_B3_CTL,
...
static void sitar_codec_calibrate_hs_polling(struct snd_soc_codec *codec, bool lock_acquired)
{
	u8 *n_ready, *n_cic;
	struct sitar_mbhc_btn_detect_cfg *btn_det;
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
	...
	if (!lock_acquired) {
		SITAR_ACQUIRE_LOCK(sitar->codec_resource_lock);
		sitar_codec_calibrate_rel(codec);
		SITAR_RELEASE_LOCK(sitar->codec_resource_lock);
	}  else {
		sitar_codec_calibrate_rel(codec);
	}
...
int sitar_mclk_enable(struct snd_soc_codec *codec, int mclk_enable, bool dapm)
{
	...
	if (mclk_enable) {
		...
		if (sitar->mbhc_polling_active) {
			...
			sitar_codec_calibrate_hs_polling(codec, lock_acquired);
			sitar_codec_start_hs_polling(codec);
		} else {
			sitar_codec_disable_clock_block(codec);
			sitar_codec_enable_bandgap(codec,
...
static short sitar_codec_setup_hs_polling(struct snd_soc_codec *codec)
{
	...
	if (!sitar->mbhc_cfg.micbias_always_on) {
	...
	sitar_codec_calibrate_hs_polling(codec, true);
...
static void sitar_codec_enable_mbhc_micbias(struct snd_soc_codec *codec,
					bool enable)
{
	int r;
	struct sitar_priv *sitar = snd_soc_codec_get_drvdata(codec);
	...
	if (enable) {
		SITAR_RELEASE_LOCK(sitar->codec_resource_lock);
		sitar_codec_update_cfilt_usage(codec,
				sitar->mbhc_bias_regs.cfilt_sel, 1);
		...
		snd_soc_dapm_sync(&codec->dapm);
		sitar_codec_update_cfilt_usage(codec,
				sitar->mbhc_bias_regs.cfilt_sel, 0);
		SITAR_ACQUIRE_LOCK(sitar->codec_resource_lock);
				    ...
				    enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		if (sitar->mbhc_cfg.headset_jack) {
			...
			* report release if we reported button press */
			...
			if (jack_type == SND_JACK_HEADSET)
				sitar_codec_enable_mbhc_micbias(codec, false);
		...
		if (jack_type == SND_JACK_HEADPHONE)
			...
			sitar_codec_enable_mbhc_micbias(codec, true);
		}
		if (sitar->mbhc_cfg.headset_jack) {
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/2/1, For 3 line headset detect*/
#if (defined(CONFIG_MACH_MSM8X30_VIPER) || defined(CONFIG_MACH_MSM8X30_DIABLOLTE) || defined(CONFIG_MACH_MSM8X30_NINGBO) || defined(CONFIG_MACH_MSM8X30_MODENA))
	if (mic_volt < 2400
			&& (mic_volt > 2330)) {
		invalid = true;
	}
...
static void sitar_hs_correct_gpio_plug(struct work_struct *work)
{
	...
	while (!time_after(jiffies, timeout)) {
		...
		SITAR_RELEASE_LOCK(sitar->codec_resource_lock);
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/1/17, Fix the platform bug of codec for viper*/
#ifdef CONFIG_TCT_8X30_COMMON
		if (sitar->hs_detect_work_stop) {
			pr_debug("%s: stop requested\n", __func__);
			break;
...
static int sitar_mbhc_init_and_calibrate(struct snd_soc_codec *codec)
{
	...
	sitar->mbhc_cfg.mclk_cb_fn(codec, 0, false);
	sitar_codec_calibrate_hs_polling(codec, false);
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/11/28, Fix the platform bug of codec for viper*/
#ifdef CONFIG_TCT_8X30_COMMON
        sitar->mbhc_cfg.calibration = (void *)fw->data;
#else
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/4/10, avoid invalid hook key when remove headset for viper*/
#ifdef CONFIG_TCT_8X30_COMMON
	printk("determine_button: bias_mv=%d\n", bias_mv);
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
	if (btn == -1)
