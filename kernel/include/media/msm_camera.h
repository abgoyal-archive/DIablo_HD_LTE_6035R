...
/* 08/01/2013|HuJin                 | add  OTP support                        */
/* 09/10/2013|HuJin                 | do some OTP optimize                    */
/* ----------|----------------------|----------------------|----------------- */
/******************************************************************************/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.HJ, 2013/08/14, add OTP support*/
#ifdef CONFIG_S5K4H5YA_TCT_EEPROM
#define CALIB_LSC_SIZE (9*7)//(17*13)
struct msm_calib_lsc {
	uint16_t r_gain[CALIB_LSC_SIZE];
	uint16_t b_gain[CALIB_LSC_SIZE];
	uint16_t gr_gain[CALIB_LSC_SIZE];
	uint16_t gb_gain[CALIB_LSC_SIZE];
};
#else
struct msm_calib_lsc {
	uint16_t r_gain[221];
...
/*[PLATFORM]-Add-BEGIN by TCTNB.HJ, 2013/08/14, add  OTP support*/
#ifdef CONFIG_S5K4H5YA_TCT_EEPROM
enum msm_camera_eeprom_info_t_SN{
// should keep same with the "msm_camera_eeprom_info_t"
#define SIZE_NORMAL sizeof(struct msm_camera_eeprom_info_t)/sizeof(struct msm_eeprom_support)
	FLAG_AF,
	FLAG_WB,
	FLAG_LSC,
	FLAG_NORMAL = SIZE_NORMAL,
};
#endif
/*[PLATFORM]-Add-END by TCTNB.HJ*/
