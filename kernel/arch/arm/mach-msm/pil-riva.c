...
#include <linux/clk.h>
/*[FEATURE]-Add-BEGIN by TCTNB.(Xinjian Fang), 2013/04/24, for CR443080 Check efuse*/
#include <linux/fs.h>
#include <asm/uaccess.h>
#include <linux/err.h>
#include "mach/scm.h"
/*[FEATURE]-Add-END by TCTNB.(Xinjian Fang)*/
...
/*#ifdef 0
    ...
    if(ret == 0)
    {
        fp = filp_open("/data/secure.fuse", O_CREAT | O_RDWR, S_IRWXU);
        if(!fp)
            printk(KERN_INFO"OPEN secure.fuse ERROR");
        else
        {
            old_fs=get_fs();
            set_fs(KERNEL_DS);
            vfs_write(fp, (const char *)scmret, sizeof(scmret), &pos);
            set_fs(old_fs);
        }
