...
//[PLATFORM]-Mod-BEGIN by TCTNB.(FLF), FR-402412, 2013/03/12
static void set_dload_mode_re(int on)
{
    if (dload_mode_addr) {
        __raw_writel(on ? 0xE47B337D : 0, dload_mode_addr);
        __raw_writel(on ? (0xCE14091A<<1) : 0,
               dload_mode_addr + sizeof(unsigned int));
#ifdef CONFIG_LGE_CRASH_HANDLER
		__raw_writel(on ? LGE_ERROR_HANDLER_MAGIC_NUM : 0,
				lge_error_handler_cookie_addr);
...
//[PLATFORM]-Add-END by TCTNB.(FLF)
	...
	/* CR-493685, enable software download even if download_mode disabled */
	if (restart_mode == RESTART_DLOAD) {
		set_dload_mode_re(1);
	}
#endif
...
/*[FEATURE]-Modify-BEGIN by TCTNB.(FLF), 2013/01/21 FR318659, Power off charging */
    else {
	    __raw_writel(0x776655AA, restart_reason);   //normal restart
/*[FEATURE]-Modify-END by TCTNB.(FLF)*/
	}
#ifdef CONFIG_LGE_CRASH_HANDLER
	if (in_panic == 1)
