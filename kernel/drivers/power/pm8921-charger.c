...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2013/04/16,add charging enable/disable interface for fr440435*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/5, refer to bug500591*/
...
/* [FEATURE]-Add-BEGIN by TCTNB.YuBin, 2013/08/14, Temperature control during charging,FR503138 */
	int				hot_hysteresis_temp_dc;
	int				cold_hysteresis_temp_dc;
/*[PLATFORM]-Add-END by TCTNB.YuBin*/
...
struct delayed_work set_tp_work;
static void set_tp_worker(struct work_struct *work)
{

	if( current_chg_status == 1 ){
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/16, Add battery ID for main&second supply*/
#ifdef CONFIG_MACH_MSM8X30_DIABLOLTE
#define MAIN_BATT_ID_MIN 1000000
#define MAIN_BATT_ID_MAX 1400000
#define SEC_BATT_ID_MIN  200000
#define SEC_BATT_ID_MAX  400000
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/16, Add battery ID for main&second supply*/
#ifdef CONFIG_MACH_MSM8X30_DIABLOLTE
	if ((rc > MAIN_BATT_ID_MIN && rc < MAIN_BATT_ID_MAX)
			|| (rc > SEC_BATT_ID_MIN && rc < SEC_BATT_ID_MAX)) {
		return 1;
	}

	pr_err("batt_id phy =%lld is invalid\n", rc);
	return 0;
#else
...
/* [FEATURE]-Add-BEGIN by TCTNB.Alvin, 2013/03/19, Keep 99% if the status is not FULL, and keep 1% if not power off */
#ifdef CONFIG_TCT_8X30_COMMON
	pr_debug("percent_soc: %d, status: %d\n", percent_soc, get_prop_batt_status(chip));
	if((percent_soc == 100) && (get_prop_batt_status(chip) == POWER_SUPPLY_STATUS_CHARGING))
	{
		pr_debug("Still in charging, set percent as 99%%\n");
		percent_soc = 99;
	}
	if (percent_soc == 0)
		percent_soc = 1;
#endif
/* [FEATURE]-Add-BEGIN by TCTNB.Alvin */
...
/* [FEATURE]-Add-BEGIN by TCTNB.Alvin, 2012/12/13, BUG#731233. For NPI MMI test, fix the temp at 25 Centi-Deg to avoid power off */
#ifdef FEATURE_TCTNB_MMITEST
    result.physical = 250;
#endif
/* [FEATURE]-Add-END by TCTNB.Alvin */
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WPL, 2013/01/11, Add control for USB instert/remove, refer to bug386091*/
#ifdef CONFIG_TCT_8X30_COMMON
	schedule_delayed_work(&set_tp_work, 0);
#endif
/*[PLATFORM]-Add-END by TCTNB.WPL*/
#endif
/*[PLATFORM]-Mod-END by TCTNB.YJ*/
...
static void btc_override_worker(struct work_struct *work)
{
	...
	if (temp) {
		if (decidegc < chip->btc_override_hot_decidegc -
				chip->hot_hysteresis_temp_dc)/* [FEATURE]-Add-BEGIN by TCTNB.YuBin, 2013/08/14, Temperature control during charging,FR503138 */
	...
	if (temp) {
		if (decidegc > chip->btc_override_cold_decidegc +
				chip->cold_hysteresis_temp_dc)/* [FEATURE]-Add-BEGIN by TCTNB.YuBin, 2013/08/14, Temperature control during charging,FR503138 */
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/05/16, Use the default safety charge time */
#ifdef CONFIG_TCT_8X30_COMMON
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2013/6/27, Delay 3S for avoid resume when using wifi hotswap*/
#ifdef CONFIG_TCT_8X30_COMMON
	schedule_delayed_work(&chip->update_heartbeat_work,
				round_jiffies_relative(msecs_to_jiffies
				(3000)));
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2013/04/16,add charging enable/disable interface for fr440435*/
#if (defined(CONFIG_MACH_MSM8X30_DIABLOLTE) || defined(CONFIG_MACH_MSM8X30_NINGBO))
#define CHARGING_DEV_IOCTL_CMD _IOWR('n', 88, int)
struct miscdevice charging_dev;

static int charging_dev_open(struct inode *inode, struct file *filp)
{
        return 0;
}

static long charging_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
        void __user *argp = (void __user *)arg;
        int charging;

        switch (cmd){
                case CHARGING_DEV_IOCTL_CMD:
                        if (copy_from_user(&charging, argp, sizeof(charging)) != 0)
                        {
                        ...
                        else if(charging == 1)
                        {
                                ...
                                {
                                        pr_err("%s battery is not in\n", __func__);
                                }
                                else
                                {
                                        pr_info("%s charging enable\n", __func__);
                        ...
                        {
                                pr_err("%s arg parameter is not right\n", __func__);
                        }
                break;
                default:
                        pr_err("%s cmd parameter is not right\n", __func__);
                break;
        }
...
static struct file_operations charging_dev_fops = {
        .owner =    THIS_MODULE,
        .unlocked_ioctl =  charging_dev_ioctl,
        .open =     charging_dev_open,
};
#endif
/*[PLATFORM]-Add-END by TCTNB.YuBin*/
...
/* [FEATURE]-Add-BEGIN by TCTNB.YuBin, 2013/08/14, Temperature control during charging,FR503138 */
	if (pdata->hot_hysteresis_temp)
		chip->hot_hysteresis_temp_dc = pdata->hot_hysteresis_temp * 10;
	else
		chip->hot_hysteresis_temp_dc = TEMP_HYSTERISIS_DECIDEGC;

	if (pdata->cold_hysteresis_temp)
		chip->cold_hysteresis_temp_dc = pdata->cold_hysteresis_temp * 10;
	else
		chip->cold_hysteresis_temp_dc = TEMP_HYSTERISIS_DECIDEGC;
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WPL, 2013/01/11, Add control for USB instert/remove, refer to bug386091*/
#ifdef CONFIG_TCT_8X30_COMMON
	INIT_DELAYED_WORK(&set_tp_work, set_tp_worker);
#endif
/*[PLATFORM]-Add-END by TCTNB.WPL*/
#endif
/*[PLATFORM]-Mod-END by TCTNB.YJ*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2013/04/16,add charging enable/disable interface for fr440435*/
#if (defined(CONFIG_MACH_MSM8X30_DIABLOLTE) || defined(CONFIG_MACH_MSM8X30_NINGBO))
        charging_dev.minor = MISC_DYNAMIC_MINOR;
        charging_dev.name = "chging_dev";
        charging_dev.fops = &charging_dev_fops;

        if (misc_register(&charging_dev)) {
                pr_err("chging_dev misc_register failed.\n.");
                goto unregister_charging_dev;
        }
#endif
/*[PLATFORM]-Add-END by TCTNB.YuBin*/
