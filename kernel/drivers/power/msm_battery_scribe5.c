...
static u32 msm_calculate_batt_capacity(u32 current_voltage)
{
        ...
        DBG_LIMIT(" ---msm_calculate_batt_capacity: current_voltage=%d \n",current_voltage);
...
static void msm_batt_update_psy_status(void)
{
        ...
        static s32 battery_full_count = 0;
        u32        charger_status = CHARGER_STATUS_INVALID;
        u32        charger_type;
        u32        battery_status;
        u32        battery_level;
        u32     battery_voltage;
        static u32     battery_voltage_pre = 0;
        s32        battery_temp;
...
/* proximity the voltage */
        if((msm_batt_info.is_resumed == true)&&(charger_status == CHARGER_STATUS_INVALID))
        {
                battery_voltage_pre = battery_voltage;//when notcharging and resumed, display the voltage at once
        }
        else
        {
                if(!battery_voltage_pre)
                        battery_voltage_pre = battery_voltage;
                else
                {
                        if(battery_voltage_pre > battery_voltage)
                                battery_voltage_pre -= 1+(battery_voltage_pre<3800);
                        else if(battery_voltage_pre < battery_voltage)
                                battery_voltage_pre += 1+(battery_voltage_pre<3800);

                        battery_voltage = battery_voltage_pre;
                }
        }
...
/* battery_temp control when charging */
        if(charger_status == CHARGER_STATUS_GOOD)
        {
                ...
                else if((1 == msm_batt_info.is_45_stop_charging)&&((battery_temp <= 43)||(battery_temp >= 0)))
                {
                ...
                if(battery_voltage >= 4350)
                {
                        if(is_battery_full())
                        {
                                if((++battery_full_count)>5)
                                {
                                        battery_full_count = 5;
...
/*
        if(is_battery_full()) {
                if((++battery_full_count)>10)
                {
                        battery_full_count = 10;
                        battery_voltage = 4350;
                }
        } else {        
                ...
                if(msm_batt_info.charger_type == charger_type && battery_full_count == 0)
                {
                        if(battery_voltage >= 4300)
                                battery_voltage = 4299;
                }
                else if(msm_batt_info.charger_type != charger_type)
                        battery_full_count = 10;
                }
*/
                DBG_LIMIT("battery_full_count=%d  battery_voltage=%d\n",battery_full_count,battery_voltage);
        }
        else
        {
                battery_full_count = 0;
        ...
        if(msm_batt_info.chg_evt_count > 0)
        {
                msm_batt_info.chg_evt_count--;
                battery_voltage_pre = msm_batt_info.battery_voltage;
        }
        else
        {
                if (msm_batt_info.battery_voltage != battery_voltage) 
                {
                        if((charger_status != CHARGER_STATUS_GOOD)&&(battery_voltage>=msm_batt_info.battery_voltage))
                                battery_voltage_pre = msm_batt_info.battery_voltage;//if not charging, and battery voltage up, donothing.
                        else if((charger_status == CHARGER_STATUS_GOOD)&&(battery_voltage<=msm_batt_info.battery_voltage)&&(battery_voltage>=msm_batt_info.battery_voltage-200))
                                battery_voltage_pre = msm_batt_info.battery_voltage;//if charging, and battery voltage down less than 200mV, donothing.
                        else
                        {
        ...
        DBG_LIMIT("BATT: chg_evt_count = %d,  voltage = %u mV [capacity = %d%%]\n",msm_batt_info.chg_evt_count,msm_batt_info.battery_voltage, msm_batt_info.batt_capacity);
        ...
        if(is_awake||(charger_status != CHARGER_STATUS_INVALID)) {
