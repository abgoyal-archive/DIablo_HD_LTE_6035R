...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WPL, 2013/01/17, refer to bug390597*/
#if (defined(CONFIG_MACH_MSM8X30_VIPER) || defined(CONFIG_MACH_MSM8X30_SCRIBELTE) || defined(CONFIG_MACH_MSM8X30_DIABLOLTE) || defined(CONFIG_MACH_MSM8X30_NINGBO)  || defined(CONFIG_MACH_MSM8X30_MODENA))
...
static unsigned char CTPM_FW_OGS[] = {
#include "ft5x06_firmware_upgrade_ogs.h"
};

static unsigned char CTPM_FW_TDT[] = {
#include "ft5x06_firmware_upgrade_tdt.h"
};
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WPL, 2013/01/17, refer to bug390597*/
	...
	if(TP_VENDOR == IS_MUTTO_TP){
		...
		CTPM_FW = CTPM_FW_OGS;
		fw_len = sizeof(CTPM_FW_OGS);
	}else if (TP_VENDOR == IS_TDT_TP){
		CTPM_FW = CTPM_FW_TDT;
		fw_len = sizeof(CTPM_FW_TDT);
    	}else{
		printk("\n\n<%s>, UNKOWN TP, return err\n\n", __func__);
		return -EIO;
	}
#else
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WPL, 2013/01/17, refer to bug390597*/
	...
	if(TP_VENDOR == IS_MUTTO_TP){
		...
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_OGS));
	}else if(TP_VENDOR == IS_TDT_TP){
		i_ret = fts_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW_TDT));
	}else{
		printk("\n\n<%s>, UNKOWN TP, return err\n\n", __func__);
		return -EIO;
	}
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WPL, 2013/01/17, refer to bug390597*/
	...
	if(TP_VENDOR == IS_MUTTO_TP){
		...
		ui_sz = sizeof(CTPM_FW_OGS);
	}else if(TP_VENDOR == IS_TDT_TP){
		ui_sz = sizeof(CTPM_FW_TDT);
    }else{
		printk("\n\n<%s>, UNKOWN TP, return err\n\n", __func__);
		return -EIO;
	}
