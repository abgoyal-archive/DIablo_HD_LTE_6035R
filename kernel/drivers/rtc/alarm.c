...
/* [FEATURE]-ADD- BEGIN by TCTNB.XQJ, 2013/07/01, FR#456484, transfer POWER OFF state alarm */
/** 
 * alarm_set_rtc - set the kernel and rtc walltime
 * @new_time:	timespec value containing the new time ,it need add current time
 */
...
/* [FEATURE]-MOD- by TCTNB.XQJ, 2013/07/04, FR#456484,add for when in off charging mobile,enable rtc,
only about  off charger function file ,charger.c will use ioctl to call this part code,not impact on normal off alarm
*/
       if(new_time.tv_sec==0 && new_time.tv_nsec==0)
       {
           pr_alarm(TSET, "alarm_set_rtc_alarm: "
                    "enable \n");
...
/* [FEATURE]-END by TCTNB.XQJ*/
	rtc_alarm.enabled = 1;
/* [FEATURE]-MOD- by TCTNB.XQJ, 2013/06/06, FR#456484,change time compute more easy,for framework */
	rtc_read_time(alarm_rtc_dev, &rtc_current_rtc_time);
	rtc_tm_to_time(&rtc_current_rtc_time,&rtc_current_time);
	new_time.tv_sec=new_time.tv_sec+rtc_current_time;
        rtc_time_to_tm(new_time.tv_sec, &rtc_alarm.time);//test
/* [FEATURE]-END by TCTNB.XQJ*/	
	...
	if (ret < 0)
		pr_alarm(ERROR, "alarm_set_rtc_alarm: "
			"Failed to set power off alarm \n");
