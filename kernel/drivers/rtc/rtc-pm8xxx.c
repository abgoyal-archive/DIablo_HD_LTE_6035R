...
#ifdef CONFIG_TCT_8X30_COMMON /* [FEATURE]-ADD- BEGIN by TCTNB.XQJ, 2013/07/01, FR#456484, trnasfer for off charger alarm */	
...
#ifdef CONFIG_TCT_8X30_COMMON /* [FEATURE]-ADD- BEGIN by TCTNB.XQJ, 2013/07/01, FR#456484, for off charger alarm */	
...
static irqreturn_t pm8xxx_alarm_trigger(int irq, void *dev_id)
{
	...
	if (rc < 0)
		dev_err(rtc_dd->rtc_dev, "PM8xxx write failed!\n");
#ifdef CONFIG_TCT_8X30_COMMON /* [FEATURE]-ADD- BEGIN by TCTNB.XQJ, 2013/07/01, FR#456484, for off charger alarm */	
	 input_report_key(rtc_dd->ip_dev, BTN_TRIGGER_HAPPY40, 1);
        input_sync(rtc_dd->ip_dev);
#endif
/* [FEATURE]-MOD- END */	
rtc_alarm_handled:
	return IRQ_HANDLED;
...
#ifdef CONFIG_TCT_8X30_COMMON /* [FEATURE]-ADD- BEGIN by TCTNB.XQJ, 2013/07/01, FR#461112, for off charger alarm */	
	rtc_dd->ip_dev = input_allocate_device();
	if (!rtc_dd->ip_dev) {
		pr_err(
			"[off alarm  error]%s: could not allocate rtc alarm  input device\n",
			__func__);
			input_free_device(rtc_dd->ip_dev);
	
	}
	rtc_dd->ip_dev->name = "off-alarm";
      input_set_capability(rtc_dd->ip_dev, EV_KEY, BTN_TRIGGER_HAPPY40);
	rc = input_register_device(rtc_dd->ip_dev);
	if (rc < 0) {
		pr_err(
			"[off alarm  error]%s: could not allocate rtc alarm  input device\n",
			__func__);
