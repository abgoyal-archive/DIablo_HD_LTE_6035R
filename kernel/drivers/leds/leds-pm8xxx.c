...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/10/25, Modify the keyboard backlight control for viper, spyder*/
#ifdef CONFIG_TCT_8X30_COMMON
#include <linux/mfd/pm8xxx/mpp.h>
#define PM8038_MPP_BASE (152+38)
#define KEYPAD_BACKLIGHT_MPP (3-1+PM8038_MPP_BASE) // compatible with pm8917
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/

/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/3/19, Add the torch control for viper, spyder*/
#ifdef CONFIG_TCT_8X30_COMMON
...
#define FLASH_MODE_GPIO_OFF 0
#define FLASH_MODE_GPIO_ON  1
#define FLASH_MODE_PWM      2
//#define PM8038_GPIO_BASE 152
//#define FLASH_EN_PM_GPIO	(1-1+PM8038_GPIO_BASE)
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/9, refer to bug500559*/
#ifdef CONFIG_MACH_MSM8X30_DIABLOLTE
#define WLED_DUMMY_HARF_LEVEL	89
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
//[PLATFORM]-Mod-BEGIN by TCTNB.(FLF), PR-486714, 2013/07/22
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/10/25, Modify the keyboard backlight control for viper, spyder*/
#ifdef CONFIG_TCT_8X30_COMMON
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/10/25, Modify the keyboard backlight control for viper, spyder*/
    ...
    {
        if (value == 1) {
            keyboard_backlight_on_config.level = PM8XXX_MPP_CS_OUT_5MA;
        } else if (value == 2) {
            keyboard_backlight_on_config.level = PM8XXX_MPP_CS_OUT_10MA;
        } else if (value == 3) {
            keyboard_backlight_on_config.level = PM8XXX_MPP_CS_OUT_15MA;
        } else if (value == 4) {
            keyboard_backlight_on_config.level = PM8XXX_MPP_CS_OUT_20MA;
        } else if (value == 5) {
            keyboard_backlight_on_config.level = PM8XXX_MPP_CS_OUT_25MA;
        } else if (value == 6) {
            keyboard_backlight_on_config.level = PM8XXX_MPP_CS_OUT_30MA;
        } else if (value == 7) {
            keyboard_backlight_on_config.level = PM8XXX_MPP_CS_OUT_35MA;
        } else if (value == 8) {
            keyboard_backlight_on_config.level = PM8XXX_MPP_CS_OUT_40MA;
        }
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/3/19, Add the camera flash led control for viper V01*/
#ifdef CONFIG_TCT_8X30_COMMON
extern void msm8930_pm8038_flashled_mode_config(int mode);
void led_flash_set_brightness(enum led_brightness value)
{
    int rc;
    int duty_us;
    static struct pwm_device *led_pwm = NULL;
    static int mode = FLASH_MODE_PWM;

    if (led_pwm == NULL) {
        led_pwm = pwm_request(0, "flash_led");
        if (IS_ERR(led_pwm)) {
            printk("flash led pwm_request failed\n");
        }
    }

    if (value == LED_FULL) {
        mode = FLASH_MODE_GPIO_ON;
        msm8930_pm8038_flashled_mode_config(mode);
    } else if (value > 0) {
        mode = FLASH_MODE_PWM;
        msm8930_pm8038_flashled_mode_config(mode);

        duty_us = value * PERIOD_US / 255;
        if (duty_us > PERIOD_US) {
            duty_us = PERIOD_US;
        }
        rc = pwm_config(led_pwm, duty_us, PERIOD_US);
        if (rc != 0) {
            printk("flash_led pwm_config fail rc=%d\n", rc);
        }
        rc = pwm_enable(led_pwm);
        if (rc != 0) {
            printk("flash_led pwm_enable fail rc=%d\n", rc);
        }
		/* Make sure the flash led is on */
		rc = pwm_config(led_pwm, duty_us, PERIOD_US);
		rc = pwm_enable(led_pwm);
    } else {
        if (mode == FLASH_MODE_GPIO_ON) {
            mode = FLASH_MODE_GPIO_OFF;
            msm8930_pm8038_flashled_mode_config(mode);
        } else {
            pwm_disable(led_pwm);
        }
...
{
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/1/2, Add the torch mode control for viper, spyder*/
#ifdef CONFIG_TCT_8X30_COMMON
//[PLATFORM]-Mod-BEGIN by TCTNB.ZL, 2013/02/19, Avoid to affect camera flash led control*/
#if 0
	int rc;
    ...
    gpio_tlmm_config(GPIO_CFG(FLASH_MODE_GPIO, 0, GPIO_CFG_OUTPUT,
                              GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
    gpio_direction_output(FLASH_MODE_GPIO, 0);
    gpio_free(FLASH_MODE_GPIO);
#endif
//[PLATFORM]-Mod-END by TCTNB.ZL, 2013/02/19*/
    led_flash_set_brightness(value);
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/8/9, refer to bug500559*/
		...
		if (value <= (LED_HALF + 1))
			...
			value = WLED_DUMMY_HARF_LEVEL + (value - LED_HALF) * 13 / 10;
...
static int __devinit init_wled(struct pm8xxx_led_data *led)
{
	...
	for (i = 0; i < num_wled_strings; i++) {
/* [PLATFORM]-Mod-BEGIN by TCTNB.Alvin, 2012/12/06, Fix the bug of Qcm WLED_MAX_CURR(Case#01039653) */
		rc = pm8xxx_readb(led->dev->parent,
...
//      if (led_cfg->default_state)
		if (led_cfg->default_state && !alarm_boot)
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.YJ, 2013/1/22, enable the LCD backlight  */
//			schedule_work(&led->work);
