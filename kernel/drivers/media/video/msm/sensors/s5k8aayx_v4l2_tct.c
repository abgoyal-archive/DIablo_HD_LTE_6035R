/* Copyright (c) 2011-2012, Code Aurora Forum. All rights reserved.
 *
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 and
 * only version 2 as published by the Free Software Foundation.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 *         Modify History For This Module
 * When           Who             What,Where,Why
 * --------------------------------------------------------------------------------------
 * 12/12/21      Hu Jin      Add driver for spyderLTE
 * 13/04/03      Hu Jin      Mod driver for diabloLTE
 * 13/06/15      pengdong.su Mod driver for diabloLTE
 * 13/07/18      Hu Jin      Import 1st tuning parameters
 * 13/07/26      Hu Jin	 The photos more noise than the preview
 * 13/08/08      Hu Jin	 optimize time cost
 */

#include "msm_sensor.h"
#include <media/msm_camera.h>
#include "s5k8aayx_v4l2_tct.h"
#include "msm.h"
#define SENSOR_NAME "s5k8aayx"
#define PLATFORM_DRIVER_NAME "msm_camera_s5k8aayx"


DEFINE_MUTEX(s5k8aayx_mut);
static struct msm_sensor_ctrl_t s5k8aayx_s_ctrl;

static struct msm_camera_i2c_reg_conf s5k8aayx_start_settings[] = {
#if 1 //hj
#else
	{0x0028, 0x7000},
	{0x002A, 0x040C},
	{0x0F12, 0x0001},
#endif
};

static struct msm_camera_i2c_reg_conf s5k8aayx_stop_settings[] = {
#if 1 //hj
#else
	{0x0028, 0x7000},
	{0x002A, 0x040C},
	{0x0F12, 0x0000},
#endif
};
#if 0
static struct msm_camera_i2c_reg_conf s5k8aayx_groupon_settings[] = {
	{0x3208, 0x00},
};

static struct msm_camera_i2c_reg_conf s5k8aayx_groupoff_settings[] = {
	{0x3208, 0x10},
	{0x3208, 0xA0},
};
#endif
static struct msm_camera_i2c_reg_conf s5k8aayx_prev_settings[] = {
	{0x0028, 0x7000},
	{0x002A, 0x01A8},
	{0x0F12, 0x0000},// REG_TC_GP_ActivePreviewConfig */
	{0x002A, 0x01AC},
	{0x0F12, 0x0001},// REG_TC_GP_PrevOpenAfterChange */
	{0x002A, 0x01A6},
	{0x0F12, 0x0001},// REG_TC_GP_NewConfigSync */
	{0x002A, 0x01AA},
	{0x0F12, 0x0001},// REG_TC_GP_PreviewConfigChanged */
	{0x002A, 0x019E},
	{0x0F12, 0x0001},// REG_TC_GP_EnablePreview */
	{0x0F12, 0x0001},// REG_TC_GP_EnablePreviewChanged */

};
static struct msm_camera_i2c_reg_conf s5k8aayx_snap_settings[] = {
	{0x0028, 0x7000},
	{0x002A, 0x01B0},
	{0x0F12, 0x0000},  //config select   0 :48, 1, 20,
	{0x002a, 0x01A6},
	{0x0F12, 0x0001},
	{0x002a, 0x01B2},
	{0x0F12, 0x0001},
	{0x002a, 0x01A2},
	{0x0F12, 0x0001},
	{0x002a, 0x01A4},
	{0x0F12, 0x0001},
};
static struct msm_camera_i2c_reg_conf s5k8aayx_recommend_settings_0[] = {
    {0xFCFC, 0xD000},//
    {0x0010, 0x0001},// // S/W Reset
    {0xFCFC, 0x0000},//
    {0x0000, 0x0000},// // Simmian bug workaround

    {0xFCFC, 0xD000},//
    {0x1030, 0x0000},// // contint_host_int
};

static struct msm_camera_i2c_reg_conf s5k8aayx_recommend_settings_1[] = {
	{0x0014, 0x0001},
	//mdelay(10);
};

static struct msm_camera_i2c_reg_conf s5k8aayx_recommend_settings_2[] = {
    //*************************************/
    // 03.Trap and Patch                  */
    //*************************************/
    // Start T&P part
    // DO NOT DELETE T&P SECTION COMMENTS! They are required to debug T&P related issues.
    // https://svn/svn/SVNRoot/System/Software/tcevb/SDK+FW/ISP_8AA/Firmware
    // SVN Rev: 42166-42166
    // ROM Rev: 
    // Signature:
    // md5 08808857c253b85b272ad1bae6844f68 .btp
    // md5 6c3dd92aafd427ef96ddcecb8d9a0dcd .htp
    // md5 92f4d9343102ef4aa626c999a25b1be9 .RegsMap.h
    // md5 6c9c1c1457ead489e12ee0feebb3adf1 .RegsMap.bin
    // md5 9d27e1e105fd2ae52c3f25336e875aa3 .base.RegsMap.h
    // md5 093c743f5c4be55ad550ddf32d7d8512 .base.RegsMap.bin
    //
    {0x0028, 0x7000},
    {0x002A, 0x2470},
    {0x0F12, 0xB510},
    {0x0F12, 0x4910},
    {0x0F12, 0x4810},
    {0x0F12, 0xF000},
    {0x0F12, 0xFA41},
    {0x0F12, 0x4910},
    {0x0F12, 0x4810},
    {0x0F12, 0xF000},
    {0x0F12, 0xFA3D},
    {0x0F12, 0x4910},
    {0x0F12, 0x4810},
    {0x0F12, 0x6341},
    {0x0F12, 0x4910},
    {0x0F12, 0x4811},
    {0x0F12, 0xF000},
    {0x0F12, 0xFA36},
    {0x0F12, 0x4910},
    {0x0F12, 0x4811},
    {0x0F12, 0xF000},
    {0x0F12, 0xFA32},
    {0x0F12, 0x4910},
    {0x0F12, 0x4811},
    {0x0F12, 0xF000},
    {0x0F12, 0xFA2E},
    {0x0F12, 0x4810},
    {0x0F12, 0x4911},
    {0x0F12, 0x6448},
    {0x0F12, 0x4911},
    {0x0F12, 0x4811},
    {0x0F12, 0xF000},
    {0x0F12, 0xFA27},
    {0x0F12, 0xBC10},
    {0x0F12, 0xBC08},
    {0x0F12, 0x4718},
    {0x0F12, 0x2870},
    {0x0F12, 0x7000},
    {0x0F12, 0x8EDD},
    {0x0F12, 0x0000},
    {0x0F12, 0x27E8},
    {0x0F12, 0x7000},
    {0x0F12, 0x8725},
    {0x0F12, 0x0000},
    {0x0F12, 0x2788},
    {0x0F12, 0x7000},
    {0x0F12, 0x0080},
    {0x0F12, 0x7000},
    {0x0F12, 0x26DC},
    {0x0F12, 0x7000},
    {0x0F12, 0xA6EF},
    {0x0F12, 0x0000},
    {0x0F12, 0x26A8},
    {0x0F12, 0x7000},
    {0x0F12, 0xA0F1},
    {0x0F12, 0x0000},
    {0x0F12, 0x2674},
    {0x0F12, 0x7000},
    {0x0F12, 0x058F},
    {0x0F12, 0x0000},
    {0x0F12, 0x2568},
    {0x0F12, 0x7000},
    {0x0F12, 0x0000},
    {0x0F12, 0x7000},
    {0x0F12, 0x24F4},
    {0x0F12, 0x7000},
    {0x0F12, 0xAC79},
    {0x0F12, 0x0000},
    {0x0F12, 0x4070},
    {0x0F12, 0xE92D},
    {0x0F12, 0x5000},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x23BC},
    {0x0F12, 0xE59F},
    {0x0F12, 0x00BE},
    {0x0F12, 0xE1D2},
    {0x0F12, 0x0100},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x10BC},
    {0x0F12, 0xE1D2},
    {0x0F12, 0x0000},
    {0x0F12, 0xE351},
    {0x0F12, 0x000E},
    {0x0F12, 0x0A00},
    {0x0F12, 0x33A8},
    {0x0F12, 0xE59F},
    {0x0F12, 0x14BC},
    {0x0F12, 0xE1D3},
    {0x0F12, 0x0000},
    {0x0F12, 0xE151},
    {0x0F12, 0x000A},
    {0x0F12, 0x9A00},
    {0x0F12, 0x0000},
    {0x0F12, 0xE041},
    {0x0F12, 0x11B0},
    {0x0F12, 0xE1D2},
    {0x0F12, 0x0091},
    {0x0F12, 0xE000},
    {0x0F12, 0x1034},
    {0x0F12, 0xE593},
    {0x0F12, 0x0520},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x0091},
    {0x0F12, 0xE000},
    {0x0F12, 0x1384},
    {0x0F12, 0xE59F},
    {0x0F12, 0x1008},
    {0x0F12, 0xE591},
    {0x0F12, 0x00F0},
    {0x0F12, 0xEB00},
    {0x0F12, 0x4000},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x0000},
    {0x0F12, 0xEA00},
    {0x0F12, 0x4000},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x00EE},
    {0x0F12, 0xEB00},
    {0x0F12, 0x0004},
    {0x0F12, 0xE080},
    {0x0F12, 0x0000},
    {0x0F12, 0xE585},
    {0x0F12, 0x4070},
    {0x0F12, 0xE8BD},
    {0x0F12, 0xFF1E},
    {0x0F12, 0xE12F},
    {0x0F12, 0x401F},
    {0x0F12, 0xE92D},
    {0x0F12, 0x00EB},
    {0x0F12, 0xEB00},
    {0x0F12, 0x2000},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x1002},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x0F86},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x00EA},
    {0x0F12, 0xEB00},
    {0x0F12, 0x2010},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x1000},
    {0x0F12, 0xE28D},
    {0x0F12, 0x0E3F},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x00E9},
    {0x0F12, 0xEB00},
    {0x0F12, 0x2001},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x1002},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x0F86},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x00E2},
    {0x0F12, 0xEB00},
    {0x0F12, 0x0000},
    {0x0F12, 0xE5DD},
    {0x0F12, 0x00C3},
    {0x0F12, 0xE350},
    {0x0F12, 0x002F},
    {0x0F12, 0x1A00},
    {0x0F12, 0x0001},
    {0x0F12, 0xE5DD},
    {0x0F12, 0x003C},
    {0x0F12, 0xE350},
    {0x0F12, 0x002C},
    {0x0F12, 0x1A00},
    {0x0F12, 0x000E},
    {0x0F12, 0xE5DD},
    {0x0F12, 0x0011},
    {0x0F12, 0xE350},
    {0x0F12, 0x0029},
    {0x0F12, 0x1A00},
    {0x0F12, 0x000F},
    {0x0F12, 0xE5DD},
    {0x0F12, 0x0011},
    {0x0F12, 0xE350},
    {0x0F12, 0x0026},
    {0x0F12, 0x1A00},
    {0x0F12, 0x02E8},
    {0x0F12, 0xE59F},
    {0x0F12, 0x10BA},
    {0x0F12, 0xE1D0},
    {0x0F12, 0x0000},
    {0x0F12, 0xE351},
    {0x0F12, 0x0022},
    {0x0F12, 0x0A00},
    {0x0F12, 0x12E4},
    {0x0F12, 0xE59F},
    {0x0F12, 0x2000},
    {0x0F12, 0xE5D1},
    {0x0F12, 0x0000},
    {0x0F12, 0xE352},
    {0x0F12, 0x0002},
    {0x0F12, 0x1A00},
    {0x0F12, 0x00B8},
    {0x0F12, 0xE1D0},
    {0x0F12, 0x0000},
    {0x0F12, 0xE350},
    {0x0F12, 0x001B},
    {0x0F12, 0x0A00},
    {0x0F12, 0x0000},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x0000},
    {0x0F12, 0xE5C1},
    {0x0F12, 0x1002},
    {0x0F12, 0xE28D},
    {0x0F12, 0x0015},
    {0x0F12, 0xEA00},
    {0x0F12, 0x2000},
    {0x0F12, 0xE5D1},
    {0x0F12, 0x3001},
    {0x0F12, 0xE5D1},
    {0x0F12, 0x3403},
    {0x0F12, 0xE182},
    {0x0F12, 0xC2B0},
    {0x0F12, 0xE59F},
    {0x0F12, 0x2080},
    {0x0F12, 0xE08C},
    {0x0F12, 0xE7B4},
    {0x0F12, 0xE1D2},
    {0x0F12, 0x039E},
    {0x0F12, 0xE004},
    {0x0F12, 0xE80F},
    {0x0F12, 0xE3E0},
    {0x0F12, 0x4624},
    {0x0F12, 0xE00E},
    {0x0F12, 0x47B4},
    {0x0F12, 0xE1C2},
    {0x0F12, 0x4004},
    {0x0F12, 0xE280},
    {0x0F12, 0xC084},
    {0x0F12, 0xE08C},
    {0x0F12, 0x47B4},
    {0x0F12, 0xE1DC},
    {0x0F12, 0x0493},
    {0x0F12, 0xE004},
    {0x0F12, 0x4624},
    {0x0F12, 0xE00E},
    {0x0F12, 0x47B4},
    {0x0F12, 0xE1CC},
    {0x0F12, 0xC8B4},
    {0x0F12, 0xE1D2},
    {0x0F12, 0x039C},
    {0x0F12, 0xE003},
    {0x0F12, 0x3623},
    {0x0F12, 0xE00E},
    {0x0F12, 0x38B4},
    {0x0F12, 0xE1C2},
    {0x0F12, 0x0001},
    {0x0F12, 0xE280},
    {0x0F12, 0x1002},
    {0x0F12, 0xE281},
    {0x0F12, 0x0004},
    {0x0F12, 0xE350},
    {0x0F12, 0xFFE7},
    {0x0F12, 0xBAFF},
    {0x0F12, 0x401F},
    {0x0F12, 0xE8BD},
    {0x0F12, 0xFF1E},
    {0x0F12, 0xE12F},
    {0x0F12, 0x4010},
    {0x0F12, 0xE92D},
    {0x0F12, 0x00B1},
    {0x0F12, 0xEB00},
    {0x0F12, 0x0250},
    {0x0F12, 0xE59F},
    {0x0F12, 0x00B2},
    {0x0F12, 0xE1D0},
    {0x0F12, 0x0000},
    {0x0F12, 0xE350},
    {0x0F12, 0x0004},
    {0x0F12, 0x0A00},
    {0x0F12, 0x0080},
    {0x0F12, 0xE310},
    {0x0F12, 0x0002},
    {0x0F12, 0x1A00},
    {0x0F12, 0x123C},
    {0x0F12, 0xE59F},
    {0x0F12, 0x0001},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x0DB2},
    {0x0F12, 0xE1C1},
    {0x0F12, 0x4010},
    {0x0F12, 0xE8BD},
    {0x0F12, 0xFF1E},
    {0x0F12, 0xE12F},
    {0x0F12, 0x4010},
    {0x0F12, 0xE92D},
    {0x0F12, 0x4000},
    {0x0F12, 0xE590},
    {0x0F12, 0x0004},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x00A5},
    {0x0F12, 0xEB00},
    {0x0F12, 0x021C},
    {0x0F12, 0xE59F},
    {0x0F12, 0x0000},
    {0x0F12, 0xE5D0},
    {0x0F12, 0x0000},
    {0x0F12, 0xE350},
    {0x0F12, 0x0002},
    {0x0F12, 0x0A00},
    {0x0F12, 0x0004},
    {0x0F12, 0xE594},
    {0x0F12, 0x00A0},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x0004},
    {0x0F12, 0xE584},
    {0x0F12, 0x4010},
    {0x0F12, 0xE8BD},
    {0x0F12, 0xFF1E},
    {0x0F12, 0xE12F},
    {0x0F12, 0x4070},
    {0x0F12, 0xE92D},
    {0x0F12, 0x0000},
    {0x0F12, 0xE590},
    {0x0F12, 0x0800},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x0820},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x4041},
    {0x0F12, 0xE280},
    {0x0F12, 0x01E8},
    {0x0F12, 0xE59F},
    {0x0F12, 0x11B8},
    {0x0F12, 0xE1D0},
    {0x0F12, 0x51B6},
    {0x0F12, 0xE1D0},
    {0x0F12, 0x0005},
    {0x0F12, 0xE041},
    {0x0F12, 0x0094},
    {0x0F12, 0xE000},
    {0x0F12, 0x1D11},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x007F},
    {0x0F12, 0xEB00},
    {0x0F12, 0x11C8},
    {0x0F12, 0xE59F},
    {0x0F12, 0x1000},
    {0x0F12, 0xE5D1},
    {0x0F12, 0x0000},
    {0x0F12, 0xE351},
    {0x0F12, 0x0000},
    {0x0F12, 0x0A00},
    {0x0F12, 0x00A0},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x21B0},
    {0x0F12, 0xE59F},
    {0x0F12, 0x3FB0},
    {0x0F12, 0xE1D2},
    {0x0F12, 0x0000},
    {0x0F12, 0xE353},
    {0x0F12, 0x0003},
    {0x0F12, 0x0A00},
    {0x0F12, 0x31AC},
    {0x0F12, 0xE59F},
    {0x0F12, 0x5BB2},
    {0x0F12, 0xE1C3},
    {0x0F12, 0xC000},
    {0x0F12, 0xE085},
    {0x0F12, 0xCBB4},
    {0x0F12, 0xE1C3},
    {0x0F12, 0x0000},
    {0x0F12, 0xE351},
    {0x0F12, 0x0000},
    {0x0F12, 0x0A00},
    {0x0F12, 0x0080},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x1DBC},
    {0x0F12, 0xE1D2},
    {0x0F12, 0x3EB4},
    {0x0F12, 0xE1D2},
    {0x0F12, 0x2EB2},
    {0x0F12, 0xE1D2},
    {0x0F12, 0x0193},
    {0x0F12, 0xE001},
    {0x0F12, 0x0092},
    {0x0F12, 0xE000},
    {0x0F12, 0x2811},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x0194},
    {0x0F12, 0xE001},
    {0x0F12, 0x0092},
    {0x0F12, 0xE000},
    {0x0F12, 0x11A1},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x01A0},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x0064},
    {0x0F12, 0xEB00},
    {0x0F12, 0x1168},
    {0x0F12, 0xE59F},
    {0x0F12, 0x02B4},
    {0x0F12, 0xE1C1},
    {0x0F12, 0x4070},
    {0x0F12, 0xE8BD},
    {0x0F12, 0xFF1E},
    {0x0F12, 0xE12F},
    {0x0F12, 0x4010},
    {0x0F12, 0xE92D},
    {0x0F12, 0x0072},
    {0x0F12, 0xEB00},
    {0x0F12, 0x2150},
    {0x0F12, 0xE59F},
    {0x0F12, 0x14B0},
    {0x0F12, 0xE1D2},
    {0x0F12, 0x0080},
    {0x0F12, 0xE311},
    {0x0F12, 0x0005},
    {0x0F12, 0x0A00},
    {0x0F12, 0x0144},
    {0x0F12, 0xE59F},
    {0x0F12, 0x00B0},
    {0x0F12, 0xE1D0},
    {0x0F12, 0x0001},
    {0x0F12, 0xE350},
    {0x0F12, 0x0001},
    {0x0F12, 0x9A00},
    {0x0F12, 0x0001},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x0000},
    {0x0F12, 0xEA00},
    {0x0F12, 0x0000},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x3118},
    {0x0F12, 0xE59F},
    {0x0F12, 0x0000},
    {0x0F12, 0xE5C3},
    {0x0F12, 0x0000},
    {0x0F12, 0xE5D3},
    {0x0F12, 0x0000},
    {0x0F12, 0xE350},
    {0x0F12, 0x0003},
    {0x0F12, 0x0A00},
    {0x0F12, 0x0080},
    {0x0F12, 0xE3C1},
    {0x0F12, 0x1114},
    {0x0F12, 0xE59F},
    {0x0F12, 0x04B0},
    {0x0F12, 0xE1C2},
    {0x0F12, 0x00B2},
    {0x0F12, 0xE1C1},
    {0x0F12, 0x4010},
    {0x0F12, 0xE8BD},
    {0x0F12, 0xFF1E},
    {0x0F12, 0xE12F},
    {0x0F12, 0x41F0},
    {0x0F12, 0xE92D},
    {0x0F12, 0x1000},
    {0x0F12, 0xE590},
    {0x0F12, 0xC801},
    {0x0F12, 0xE1A0},
    {0x0F12, 0xC82C},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x1004},
    {0x0F12, 0xE590},
    {0x0F12, 0x1801},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x1821},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x4008},
    {0x0F12, 0xE590},
    {0x0F12, 0x500C},
    {0x0F12, 0xE590},
    {0x0F12, 0x2004},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x3005},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x000C},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x0052},
    {0x0F12, 0xEB00},
    {0x0F12, 0x609C},
    {0x0F12, 0xE59F},
    {0x0F12, 0x00B2},
    {0x0F12, 0xE1D6},
    {0x0F12, 0x0000},
    {0x0F12, 0xE350},
    {0x0F12, 0x000E},
    {0x0F12, 0x0A00},
    {0x0F12, 0x00C0},
    {0x0F12, 0xE59F},
    {0x0F12, 0x05B4},
    {0x0F12, 0xE1D0},
    {0x0F12, 0x0002},
    {0x0F12, 0xE350},
    {0x0F12, 0x000A},
    {0x0F12, 0x1A00},
    {0x0F12, 0x7080},
    {0x0F12, 0xE59F},
    {0x0F12, 0x10F4},
    {0x0F12, 0xE1D6},
    {0x0F12, 0x26B0},
    {0x0F12, 0xE1D7},
    {0x0F12, 0x00F0},
    {0x0F12, 0xE1D4},
    {0x0F12, 0x0048},
    {0x0F12, 0xEB00},
    {0x0F12, 0x00B0},
    {0x0F12, 0xE1C4},
    {0x0F12, 0x26B0},
    {0x0F12, 0xE1D7},
    {0x0F12, 0x10F6},
    {0x0F12, 0xE1D6},
    {0x0F12, 0x00F0},
    {0x0F12, 0xE1D5},
    {0x0F12, 0x0043},
    {0x0F12, 0xEB00},
    {0x0F12, 0x00B0},
    {0x0F12, 0xE1C5},
    {0x0F12, 0x41F0},
    {0x0F12, 0xE8BD},
    {0x0F12, 0xFF1E},
    {0x0F12, 0xE12F},
    {0x0F12, 0x4010},
    {0x0F12, 0xE92D},
    {0x0F12, 0x4000},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x1004},
    {0x0F12, 0xE594},
    {0x0F12, 0x003C},
    {0x0F12, 0xE59F},
    {0x0F12, 0x00B0},
    {0x0F12, 0xE1D0},
    {0x0F12, 0x0000},
    {0x0F12, 0xE350},
    {0x0F12, 0x0008},
    {0x0F12, 0x0A00},
    {0x0F12, 0x0030},
    {0x0F12, 0xE59F},
    {0x0F12, 0x3001},
    {0x0F12, 0xE1A0},
    {0x0F12, 0x2068},
    {0x0F12, 0xE590},
    {0x0F12, 0x0058},
    {0x0F12, 0xE59F},
    {0x0F12, 0x1005},
    {0x0F12, 0xE3A0},
    {0x0F12, 0x0036},
    {0x0F12, 0xEB00},
    {0x0F12, 0x0000},
    {0x0F12, 0xE584},
    {0x0F12, 0x4010},
    {0x0F12, 0xE8BD},
    {0x0F12, 0xFF1E},
    {0x0F12, 0xE12F},
    {0x0F12, 0x0000},
    {0x0F12, 0xE594},
    {0x0F12, 0x0034},
    {0x0F12, 0xEB00},
    {0x0F12, 0x0000},
    {0x0F12, 0xE584},
    {0x0F12, 0xFFF9},
    {0x0F12, 0xEAFF},
    {0x0F12, 0x3360},
    {0x0F12, 0x7000},
    {0x0F12, 0x20D4},
    {0x0F12, 0x7000},
    {0x0F12, 0x16C8},
    {0x0F12, 0x7000},
    {0x0F12, 0x299C},
    {0x0F12, 0x7000},
    {0x0F12, 0x1272},
    {0x0F12, 0x7000},
    {0x0F12, 0x1728},
    {0x0F12, 0x7000},
    {0x0F12, 0x112C},
    {0x0F12, 0x7000},
    {0x0F12, 0x29A0},
    {0x0F12, 0x7000},
    {0x0F12, 0x122C},
    {0x0F12, 0x7000},
    {0x0F12, 0xF200},
    {0x0F12, 0xD000},
    {0x0F12, 0x2340},
    {0x0F12, 0x7000},
    {0x0F12, 0x0E2C},
    {0x0F12, 0x7000},
    {0x0F12, 0xF400},
    {0x0F12, 0xD000},
    {0x0F12, 0x0CDC},
    {0x0F12, 0x7000},
    {0x0F12, 0x06D4},
    {0x0F12, 0x7000},
    {0x0F12, 0x4778},
    {0x0F12, 0x46C0},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0xC091},
    {0x0F12, 0x0000},
    {0x0F12, 0xF004},
    {0x0F12, 0xE51F},
    {0x0F12, 0xD14C},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0xAC79},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0x0467},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0x2FA7},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0xCB1F},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0x058F},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0xA0F1},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0x2B43},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0x8725},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0x6777},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0x8E49},
    {0x0F12, 0x0000},
    {0x0F12, 0xC000},
    {0x0F12, 0xE59F},
    {0x0F12, 0xFF1C},
    {0x0F12, 0xE12F},
    {0x0F12, 0x8EDD},
    {0x0F12, 0x0000},
    {0x0F12, 0xA4B6},
    {0x0F12, 0x0000},
    {0x0F12, 0x0001},
    {0x0F12, 0x0000},
//*********************************************************************************
// 06.GAS (Grid Anti-Shading)
//*********************************************************************************
    {0x002A, 0x1326},
    {0x0F12, 0x0000},
    {0x002A, 0x063A},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x00E8},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x00C8},
    {0x0F12, 0x00F8},
    {0x0F12, 0x00F8},
    {0x0F12, 0x0100},
    {0x0F12, 0x00F0},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x00F0},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x002A, 0x067A},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x002A, 0x06BA},
    {0x0F12, 0x0001},
    {0x002A, 0x0632},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x002A, 0x0672},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x0F12, 0x0100},
    {0x002A, 0x06B2},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x002A, 0x06D0},
    {0x0F12, 0x000D},
    {0x0F12, 0x000F},
    {0x002A, 0x06CC},
    {0x0F12, 0x0280},
    {0x0F12, 0x01E0},
    {0x002A, 0x06C6},
    {0x0F12, 0x0001},

    {0x002A, 0x0624},
    {0x0F12, 0x009D},
    {0x0F12, 0x00D5},
    {0x0F12, 0x0103},
    {0x0F12, 0x0128},
    {0x0F12, 0x0166},
    {0x0F12, 0x0193},
    {0x0F12, 0x01A0},

    {0x002A, 0x347C},
    {0x0F12, 0x013B},
    {0x0F12, 0x0116},
    {0x0F12, 0x00D9},
    {0x0F12, 0x00A6},
    {0x0F12, 0x0082},
    {0x0F12, 0x006C},
    {0x0F12, 0x0065},
    {0x0F12, 0x006C},
    {0x0F12, 0x0080},
    {0x0F12, 0x00A3},
    {0x0F12, 0x00D4},
    {0x0F12, 0x010D},
    {0x0F12, 0x012E},
    {0x0F12, 0x0138},
    {0x0F12, 0x0104},
    {0x0F12, 0x00BE},
    {0x0F12, 0x0088},
    {0x0F12, 0x0062},
    {0x0F12, 0x004D},
    {0x0F12, 0x0046},
    {0x0F12, 0x004C},
    {0x0F12, 0x0060},
    {0x0F12, 0x0084},
    {0x0F12, 0x00B8},
    {0x0F12, 0x00F9},
    {0x0F12, 0x012C},
    {0x0F12, 0x011A},
    {0x0F12, 0x00DB},
    {0x0F12, 0x0093},
    {0x0F12, 0x005F},
    {0x0F12, 0x003C},
    {0x0F12, 0x0027},
    {0x0F12, 0x0020},
    {0x0F12, 0x0026},
    {0x0F12, 0x003A},
    {0x0F12, 0x005C},
    {0x0F12, 0x008E},
    {0x0F12, 0x00D2},
    {0x0F12, 0x010E},
    {0x0F12, 0x0101},
    {0x0F12, 0x00BF},
    {0x0F12, 0x0077},
    {0x0F12, 0x0044},
    {0x0F12, 0x0023},
    {0x0F12, 0x0011},
    {0x0F12, 0x000C},
    {0x0F12, 0x0010},
    {0x0F12, 0x0022},
    {0x0F12, 0x0043},
    {0x0F12, 0x0074},
    {0x0F12, 0x00B7},
    {0x0F12, 0x00F7},
    {0x0F12, 0x00FC},
    {0x0F12, 0x00B7},
    {0x0F12, 0x006F},
    {0x0F12, 0x003C},
    {0x0F12, 0x001C},
    {0x0F12, 0x000A},
    {0x0F12, 0x0004},
    {0x0F12, 0x000A},
    {0x0F12, 0x001B},
    {0x0F12, 0x003B},
    {0x0F12, 0x006C},
    {0x0F12, 0x00B0},
    {0x0F12, 0x00F2},
    {0x0F12, 0x00EF},
    {0x0F12, 0x00AB},
    {0x0F12, 0x0065},
    {0x0F12, 0x0034},
    {0x0F12, 0x0015},
    {0x0F12, 0x0004},
    {0x0F12, 0x0000},
    {0x0F12, 0x0004},
    {0x0F12, 0x0013},
    {0x0F12, 0x0033},
    {0x0F12, 0x0063},
    {0x0F12, 0x00A5},
    {0x0F12, 0x00E5},
    {0x0F12, 0x00F7},
    {0x0F12, 0x00B4},
    {0x0F12, 0x006D},
    {0x0F12, 0x003C},
    {0x0F12, 0x001C},
    {0x0F12, 0x000B},
    {0x0F12, 0x0005},
    {0x0F12, 0x000A},
    {0x0F12, 0x001B},
    {0x0F12, 0x003B},
    {0x0F12, 0x006B},
    {0x0F12, 0x00AD},
    {0x0F12, 0x00ED},
    {0x0F12, 0x010B},
    {0x0F12, 0x00CB},
    {0x0F12, 0x0085},
    {0x0F12, 0x0051},
    {0x0F12, 0x002F},
    {0x0F12, 0x001C},
    {0x0F12, 0x0016},
    {0x0F12, 0x001C},
    {0x0F12, 0x002E},
    {0x0F12, 0x004F},
    {0x0F12, 0x0081},
    {0x0F12, 0x00C4},
    {0x0F12, 0x0102},
    {0x0F12, 0x0119},
    {0x0F12, 0x00DF},
    {0x0F12, 0x009B},
    {0x0F12, 0x0067},
    {0x0F12, 0x0045},
    {0x0F12, 0x0030},
    {0x0F12, 0x0029},
    {0x0F12, 0x002F},
    {0x0F12, 0x0043},
    {0x0F12, 0x0066},
    {0x0F12, 0x0098},
    {0x0F12, 0x00D9},
    {0x0F12, 0x010F},
    {0x0F12, 0x0138},
    {0x0F12, 0x010C},
    {0x0F12, 0x00CB},
    {0x0F12, 0x0097},
    {0x0F12, 0x0073},
    {0x0F12, 0x005C},
    {0x0F12, 0x0054},
    {0x0F12, 0x005B},
    {0x0F12, 0x0070},
    {0x0F12, 0x0096},
    {0x0F12, 0x00C9},
    {0x0F12, 0x0106},
    {0x0F12, 0x012D},
    {0x0F12, 0x0147},
    {0x0F12, 0x012F},
    {0x0F12, 0x00F8},
    {0x0F12, 0x00C5},
    {0x0F12, 0x00A1},
    {0x0F12, 0x008B},
    {0x0F12, 0x0083},
    {0x0F12, 0x008B},
    {0x0F12, 0x00A0},
    {0x0F12, 0x00C2},
    {0x0F12, 0x00F3},
    {0x0F12, 0x0124},
    {0x0F12, 0x0139},
    {0x0F12, 0x0093},
    {0x0F12, 0x007E},
    {0x0F12, 0x0062},
    {0x0F12, 0x004D},
    {0x0F12, 0x003E},
    {0x0F12, 0x0034},
    {0x0F12, 0x0030},
    {0x0F12, 0x0032},
    {0x0F12, 0x003B},
    {0x0F12, 0x0049},
    {0x0F12, 0x005C},
    {0x0F12, 0x0077},
    {0x0F12, 0x008A},
    {0x0F12, 0x0093},
    {0x0F12, 0x0077},
    {0x0F12, 0x0059},
    {0x0F12, 0x0042},
    {0x0F12, 0x0032},
    {0x0F12, 0x0027},
    {0x0F12, 0x0024},
    {0x0F12, 0x0026},
    {0x0F12, 0x002F},
    {0x0F12, 0x003D},
    {0x0F12, 0x0052},
    {0x0F12, 0x006E},
    {0x0F12, 0x008B},
    {0x0F12, 0x0083},
    {0x0F12, 0x0064},
    {0x0F12, 0x0046},
    {0x0F12, 0x0030},
    {0x0F12, 0x0020},
    {0x0F12, 0x0016},
    {0x0F12, 0x0011},
    {0x0F12, 0x0014},
    {0x0F12, 0x001E},
    {0x0F12, 0x002D},
    {0x0F12, 0x0041},
    {0x0F12, 0x005D},
    {0x0F12, 0x007C},
    {0x0F12, 0x0077},
    {0x0F12, 0x0057},
    {0x0F12, 0x0039},
    {0x0F12, 0x0024},
    {0x0F12, 0x0014},
    {0x0F12, 0x000A},
    {0x0F12, 0x0007},
    {0x0F12, 0x0009},
    {0x0F12, 0x0012},
    {0x0F12, 0x0021},
    {0x0F12, 0x0036},
    {0x0F12, 0x0051},
    {0x0F12, 0x0070},
    {0x0F12, 0x0077},
    {0x0F12, 0x0056},
    {0x0F12, 0x0038},
    {0x0F12, 0x0022},
    {0x0F12, 0x0013},
    {0x0F12, 0x0009},
    {0x0F12, 0x0005},
    {0x0F12, 0x0008},
    {0x0F12, 0x0011},
    {0x0F12, 0x0020},
    {0x0F12, 0x0035},
    {0x0F12, 0x0051},
    {0x0F12, 0x0071},
    {0x0F12, 0x006E},
    {0x0F12, 0x004E},
    {0x0F12, 0x0032},
    {0x0F12, 0x001C},
    {0x0F12, 0x000D},
    {0x0F12, 0x0004},
    {0x0F12, 0x0000},
    {0x0F12, 0x0003},
    {0x0F12, 0x000B},
    {0x0F12, 0x001A},
    {0x0F12, 0x002F},
    {0x0F12, 0x0049},
    {0x0F12, 0x0068},
    {0x0F12, 0x0072},
    {0x0F12, 0x0053},
    {0x0F12, 0x0037},
    {0x0F12, 0x0021},
    {0x0F12, 0x0012},
    {0x0F12, 0x0009},
    {0x0F12, 0x0005},
    {0x0F12, 0x0008},
    {0x0F12, 0x0010},
    {0x0F12, 0x001F},
    {0x0F12, 0x0034},
    {0x0F12, 0x004E},
    {0x0F12, 0x006C},
    {0x0F12, 0x007F},
    {0x0F12, 0x0060},
    {0x0F12, 0x0043},
    {0x0F12, 0x002D},
    {0x0F12, 0x001D},
    {0x0F12, 0x0013},
    {0x0F12, 0x0010},
    {0x0F12, 0x0013},
    {0x0F12, 0x001C},
    {0x0F12, 0x002B},
    {0x0F12, 0x0040},
    {0x0F12, 0x005A},
    {0x0F12, 0x0079},
    {0x0F12, 0x0082},
    {0x0F12, 0x0066},
    {0x0F12, 0x0049},
    {0x0F12, 0x0035},
    {0x0F12, 0x0025},
    {0x0F12, 0x001B},
    {0x0F12, 0x0017},
    {0x0F12, 0x0019},
    {0x0F12, 0x0023},
    {0x0F12, 0x0033},
    {0x0F12, 0x0046},
    {0x0F12, 0x0060},
    {0x0F12, 0x007B},
    {0x0F12, 0x0092},
    {0x0F12, 0x007C},
    {0x0F12, 0x0060},
    {0x0F12, 0x004B},
    {0x0F12, 0x003C},
    {0x0F12, 0x0032},
    {0x0F12, 0x002D},
    {0x0F12, 0x0030},
    {0x0F12, 0x0039},
    {0x0F12, 0x0049},
    {0x0F12, 0x005D},
    {0x0F12, 0x0076},
    {0x0F12, 0x008C},
    {0x0F12, 0x009F},
    {0x0F12, 0x008F},
    {0x0F12, 0x0077},
    {0x0F12, 0x0061},
    {0x0F12, 0x0052},
    {0x0F12, 0x0048},
    {0x0F12, 0x0043},
    {0x0F12, 0x0047},
    {0x0F12, 0x0050},
    {0x0F12, 0x005E},
    {0x0F12, 0x0071},
    {0x0F12, 0x0086},
    {0x0F12, 0x0097},
    {0x0F12, 0x0093},
    {0x0F12, 0x007C},
    {0x0F12, 0x005F},
    {0x0F12, 0x0049},
    {0x0F12, 0x003A},
    {0x0F12, 0x0030},
    {0x0F12, 0x002C},
    {0x0F12, 0x002F},
    {0x0F12, 0x0037},
    {0x0F12, 0x0045},
    {0x0F12, 0x005A},
    {0x0F12, 0x0075},
    {0x0F12, 0x008A},
    {0x0F12, 0x0094},
    {0x0F12, 0x0077},
    {0x0F12, 0x0057},
    {0x0F12, 0x0040},
    {0x0F12, 0x002F},
    {0x0F12, 0x0024},
    {0x0F12, 0x0020},
    {0x0F12, 0x0023},
    {0x0F12, 0x002D},
    {0x0F12, 0x003B},
    {0x0F12, 0x0051},
    {0x0F12, 0x006E},
    {0x0F12, 0x008C},
    {0x0F12, 0x0085},
    {0x0F12, 0x0066},
    {0x0F12, 0x0046},
    {0x0F12, 0x002F},
    {0x0F12, 0x001F},
    {0x0F12, 0x0014},
    {0x0F12, 0x000F},
    {0x0F12, 0x0012},
    {0x0F12, 0x001C},
    {0x0F12, 0x002B},
    {0x0F12, 0x0040},
    {0x0F12, 0x005C},
    {0x0F12, 0x007D},
    {0x0F12, 0x007A},
    {0x0F12, 0x005A},
    {0x0F12, 0x003A},
    {0x0F12, 0x0024},
    {0x0F12, 0x0014},
    {0x0F12, 0x0009},
    {0x0F12, 0x0006},
    {0x0F12, 0x0008},
    {0x0F12, 0x0011},
    {0x0F12, 0x0020},
    {0x0F12, 0x0036},
    {0x0F12, 0x0051},
    {0x0F12, 0x0072},
    {0x0F12, 0x007B},
    {0x0F12, 0x0059},
    {0x0F12, 0x003A},
    {0x0F12, 0x0023},
    {0x0F12, 0x0012},
    {0x0F12, 0x0008},
    {0x0F12, 0x0004},
    {0x0F12, 0x0007},
    {0x0F12, 0x000F},
    {0x0F12, 0x001F},
    {0x0F12, 0x0035},
    {0x0F12, 0x0051},
    {0x0F12, 0x0072},
    {0x0F12, 0x0073},
    {0x0F12, 0x0053},
    {0x0F12, 0x0034},
    {0x0F12, 0x001D},
    {0x0F12, 0x000E},
    {0x0F12, 0x0004},
    {0x0F12, 0x0000},
    {0x0F12, 0x0002},
    {0x0F12, 0x000A},
    {0x0F12, 0x001A},
    {0x0F12, 0x002F},
    {0x0F12, 0x004A},
    {0x0F12, 0x006A},
    {0x0F12, 0x0077},
    {0x0F12, 0x0058},
    {0x0F12, 0x0039},
    {0x0F12, 0x0022},
    {0x0F12, 0x0012},
    {0x0F12, 0x0008},
    {0x0F12, 0x0004},
    {0x0F12, 0x0007},
    {0x0F12, 0x000F},
    {0x0F12, 0x001E},
    {0x0F12, 0x0034},
    {0x0F12, 0x004F},
    {0x0F12, 0x006F},
    {0x0F12, 0x0083},
    {0x0F12, 0x0064},
    {0x0F12, 0x0045},
    {0x0F12, 0x002E},
    {0x0F12, 0x001D},
    {0x0F12, 0x0012},
    {0x0F12, 0x000F},
    {0x0F12, 0x0011},
    {0x0F12, 0x001A},
    {0x0F12, 0x002A},
    {0x0F12, 0x003F},
    {0x0F12, 0x005B},
    {0x0F12, 0x007B},
    {0x0F12, 0x0087},
    {0x0F12, 0x006A},
    {0x0F12, 0x004B},
    {0x0F12, 0x0036},
    {0x0F12, 0x0025},
    {0x0F12, 0x0019},
    {0x0F12, 0x0015},
    {0x0F12, 0x0017},
    {0x0F12, 0x0022},
    {0x0F12, 0x0031},
    {0x0F12, 0x0045},
    {0x0F12, 0x0060},
    {0x0F12, 0x007D},
    {0x0F12, 0x0096},
    {0x0F12, 0x007F},
    {0x0F12, 0x0061},
    {0x0F12, 0x004B},
    {0x0F12, 0x003B},
    {0x0F12, 0x002F},
    {0x0F12, 0x002A},
    {0x0F12, 0x002D},
    {0x0F12, 0x0036},
    {0x0F12, 0x0046},
    {0x0F12, 0x005B},
    {0x0F12, 0x0075},
    {0x0F12, 0x008D},
    {0x0F12, 0x00A1},
    {0x0F12, 0x0091},
    {0x0F12, 0x0077},
    {0x0F12, 0x0060},
    {0x0F12, 0x0050},
    {0x0F12, 0x0044},
    {0x0F12, 0x0040},
    {0x0F12, 0x0043},
    {0x0F12, 0x004C},
    {0x0F12, 0x005A},
    {0x0F12, 0x006D},
    {0x0F12, 0x0084},
    {0x0F12, 0x0094},
    {0x0F12, 0x0072},
    {0x0F12, 0x0063},
    {0x0F12, 0x004C},
    {0x0F12, 0x003A},
    {0x0F12, 0x002D},
    {0x0F12, 0x0025},
    {0x0F12, 0x0023},
    {0x0F12, 0x0025},
    {0x0F12, 0x002C},
    {0x0F12, 0x0038},
    {0x0F12, 0x004A},
    {0x0F12, 0x005F},
    {0x0F12, 0x006B},
    {0x0F12, 0x0079},
    {0x0F12, 0x0065},
    {0x0F12, 0x004A},
    {0x0F12, 0x0037},
    {0x0F12, 0x0029},
    {0x0F12, 0x0021},
    {0x0F12, 0x001D},
    {0x0F12, 0x001F},
    {0x0F12, 0x0027},
    {0x0F12, 0x0033},
    {0x0F12, 0x0044},
    {0x0F12, 0x005E},
    {0x0F12, 0x006E},
    {0x0F12, 0x006A},
    {0x0F12, 0x0055},
    {0x0F12, 0x003A},
    {0x0F12, 0x0028},
    {0x0F12, 0x001A},
    {0x0F12, 0x0011},
    {0x0F12, 0x000D},
    {0x0F12, 0x000F},
    {0x0F12, 0x0017},
    {0x0F12, 0x0024},
    {0x0F12, 0x0035},
    {0x0F12, 0x004E},
    {0x0F12, 0x0061},
    {0x0F12, 0x0061},
    {0x0F12, 0x004A},
    {0x0F12, 0x0031},
    {0x0F12, 0x001E},
    {0x0F12, 0x0011},
    {0x0F12, 0x0008},
    {0x0F12, 0x0005},
    {0x0F12, 0x0007},
    {0x0F12, 0x000E},
    {0x0F12, 0x001B},
    {0x0F12, 0x002D},
    {0x0F12, 0x0045},
    {0x0F12, 0x0059},
    {0x0F12, 0x0062},
    {0x0F12, 0x004B},
    {0x0F12, 0x0031},
    {0x0F12, 0x001E},
    {0x0F12, 0x0010},
    {0x0F12, 0x0008},
    {0x0F12, 0x0004},
    {0x0F12, 0x0006},
    {0x0F12, 0x000E},
    {0x0F12, 0x001B},
    {0x0F12, 0x002E},
    {0x0F12, 0x0046},
    {0x0F12, 0x005A},
    {0x0F12, 0x005B},
    {0x0F12, 0x0045},
    {0x0F12, 0x002C},
    {0x0F12, 0x001A},
    {0x0F12, 0x000C},
    {0x0F12, 0x0003},
    {0x0F12, 0x0000},
    {0x0F12, 0x0002},
    {0x0F12, 0x0009},
    {0x0F12, 0x0016},
    {0x0F12, 0x0029},
    {0x0F12, 0x0040},
    {0x0F12, 0x0054},
    {0x0F12, 0x005F},
    {0x0F12, 0x004A},
    {0x0F12, 0x0031},
    {0x0F12, 0x001F},
    {0x0F12, 0x0010},
    {0x0F12, 0x0008},
    {0x0F12, 0x0004},
    {0x0F12, 0x0007},
    {0x0F12, 0x000E},
    {0x0F12, 0x001B},
    {0x0F12, 0x002E},
    {0x0F12, 0x0045},
    {0x0F12, 0x0059},
    {0x0F12, 0x006C},
    {0x0F12, 0x0057},
    {0x0F12, 0x003E},
    {0x0F12, 0x002A},
    {0x0F12, 0x001B},
    {0x0F12, 0x0012},
    {0x0F12, 0x000F},
    {0x0F12, 0x0011},
    {0x0F12, 0x0019},
    {0x0F12, 0x0027},
    {0x0F12, 0x0039},
    {0x0F12, 0x0050},
    {0x0F12, 0x0063},
    {0x0F12, 0x006F},
    {0x0F12, 0x005C},
    {0x0F12, 0x0044},
    {0x0F12, 0x0031},
    {0x0F12, 0x0023},
    {0x0F12, 0x0019},
    {0x0F12, 0x0016},
    {0x0F12, 0x0017},
    {0x0F12, 0x0020},
    {0x0F12, 0x002E},
    {0x0F12, 0x0040},
    {0x0F12, 0x0055},
    {0x0F12, 0x0064},
    {0x0F12, 0x007E},
    {0x0F12, 0x0071},
    {0x0F12, 0x0059},
    {0x0F12, 0x0046},
    {0x0F12, 0x0039},
    {0x0F12, 0x002F},
    {0x0F12, 0x002A},
    {0x0F12, 0x002D},
    {0x0F12, 0x0035},
    {0x0F12, 0x0043},
    {0x0F12, 0x0054},
    {0x0F12, 0x0069},
    {0x0F12, 0x0074},
    {0x0F12, 0x0083},
    {0x0F12, 0x007D},
    {0x0F12, 0x0068},
    {0x0F12, 0x0055},
    {0x0F12, 0x0048},
    {0x0F12, 0x003E},
    {0x0F12, 0x003A},
    {0x0F12, 0x003D},
    {0x0F12, 0x0045},
    {0x0F12, 0x0051},
    {0x0F12, 0x0061},
    {0x0F12, 0x0072},
    {0x0F12, 0x0077},
    {0x002A, 0x1348},
    {0x0F12, 0x0001},

    {0x002A, 0x0EC6},
    {0x0F12, 0x000B},
    {0x002A, 0x0ECE},
    {0x0F12, 0x000B},
    {0x002A, 0x0F16},
    {0x0F12, 0x002B},
    {0x002A, 0x0F1E},
    {0x0F12, 0x002B},
    {0x002A, 0x0EC8},
    {0x0F12, 0x000B},
    {0x002A, 0x0ED0},
    {0x0F12, 0x033F},
    {0x002A, 0x0F18},
    {0x0F12, 0x0366},
    {0x002A, 0x0F20},
    {0x0F12, 0x0032},

    {0x002A, 0x0E38},
    {0x0F12, 0x0476},
    {0x0F12, 0x0476},
    {0x002A, 0x0AA0},
    {0x0F12, 0x0001},
    {0x002A, 0x0E2C},
    {0x0F12, 0x0001},
    {0x002A, 0x1250},
    {0x0F12, 0xFFFF},
    {0x002A, 0x1202},
    {0x0F12, 0x0010},

    //***************
    // 09.AWB-BASIC s
    //***************

    // For WB Calibra
    {0x002A, 0x0B36},
    {0x0F12, 0x0005},
    {0x002A, 0x0B3A},
    {0x0F12, 0x00F3},
    {0x0F12, 0x02CB},
    {0x002A, 0x0B38},
    {0x0F12, 0x0010},
    {0x002A, 0x0AE6},
    {0x0F12, 0x0385},
    {0x0F12, 0x03D8},
    {0x0F12, 0x032A},
    {0x0F12, 0x03C5},
    {0x0F12, 0x02F5},
    {0x0F12, 0x039D},
    {0x0F12, 0x02D3},
    {0x0F12, 0x0372},
    {0x0F12, 0x02B1},
    {0x0F12, 0x033E},
    {0x0F12, 0x028A},
    {0x0F12, 0x0322},
    {0x0F12, 0x0268},
    {0x0F12, 0x02FD},
    {0x0F12, 0x0248},
    {0x0F12, 0x02EF},
    {0x0F12, 0x022F},
    {0x0F12, 0x02D5},
    {0x0F12, 0x0219},
    {0x0F12, 0x02C2},
    {0x0F12, 0x0206},
    {0x0F12, 0x02A3},
    {0x0F12, 0x01F0},
    {0x0F12, 0x0286},
    {0x0F12, 0x01E3},
    {0x0F12, 0x0268},
    {0x0F12, 0x01D6},
    {0x0F12, 0x024E},
    {0x0F12, 0x01DD},
    {0x0F12, 0x022A},
    {0x0F12, 0x0210},
    {0x0F12, 0x01F2},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},

    {0x002A, 0x0BAA},
    {0x0F12, 0x0006},
    {0x002A, 0x0BAE},
    {0x0F12, 0x00CC},
    {0x0F12, 0x02F3},
    {0x002A, 0x0BAC},
    {0x0F12, 0x000A},
    {0x002A, 0x0B7A},
    {0x0F12, 0x036C},
    {0x0F12, 0x03C6},
    {0x0F12, 0x02EE},
    {0x0F12, 0x03F9},
    {0x0F12, 0x02BE},
    {0x0F12, 0x03DF},
    {0x0F12, 0x027A},
    {0x0F12, 0x03AE},
    {0x0F12, 0x0234},
    {0x0F12, 0x0376},
    {0x0F12, 0x0204},
    {0x0F12, 0x033E},
    {0x0F12, 0x01E0},
    {0x0F12, 0x02CD},
    {0x0F12, 0x01C3},
    {0x0F12, 0x027A},
    {0x0F12, 0x01B7},
    {0x0F12, 0x0244},
    {0x0F12, 0x01FE},
    {0x0F12, 0x01DD},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x002A, 0x0B70},
    {0x0F12, 0x0005},
    {0x002A, 0x0B74},
    {0x0F12, 0x01E3},
    {0x0F12, 0x0270},
    {0x002A, 0x0B72},
    {0x0F12, 0x0006},
    {0x002A, 0x0B40},
    {0x0F12, 0x028A},
    {0x0F12, 0x02A1},
    {0x0F12, 0x0263},
    {0x0F12, 0x02C0},
    {0x0F12, 0x024C},
    {0x0F12, 0x02BE},
    {0x0F12, 0x023D},
    {0x0F12, 0x02A6},
    {0x0F12, 0x0243},
    {0x0F12, 0x0289},
    {0x0F12, 0x026F},
    {0x0F12, 0x025D},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x002A, 0x0BC8},
    {0x0F12, 0x0005},
    {0x002A, 0x0BCC},
    {0x0F12, 0x010F},
    {0x0F12, 0x018F},
    {0x002A, 0x0BCA},
    {0x0F12, 0x0005},
    {0x002A, 0x0BB4},
    {0x0F12, 0x03E7},
    {0x0F12, 0x03F8},
    {0x0F12, 0x03A7},
    {0x0F12, 0x03FC},
    {0x0F12, 0x0352},
    {0x0F12, 0x03D0},
    {0x0F12, 0x0322},
    {0x0F12, 0x039E},
    {0x0F12, 0x032B},
    {0x0F12, 0x034D},
    {0x002A, 0x0BE6},
    {0x0F12, 0x0006},
    {0x002A, 0x0BEA},
    {0x0F12, 0x019E},
    {0x0F12, 0x0257},
    {0x002A, 0x0BE8},
    {0x0F12, 0x0004},
    {0x002A, 0x0BD2},
    {0x0F12, 0x030B},
    {0x0F12, 0x0323},
    {0x0F12, 0x02C3},
    {0x0F12, 0x030F},
    {0x0F12, 0x0288},
    {0x0F12, 0x02E5},
    {0x0F12, 0x026A},
    {0x0F12, 0x02A2},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x002A, 0x0C2C},
    {0x0F12, 0x0139},
    {0x0F12, 0x0122},
    {0x002A, 0x0BFC},
    {0x0F12, 0x0378},
    {0x0F12, 0x011E},
    {0x0F12, 0x02F0},
    {0x0F12, 0x0184},
    {0x0F12, 0x0313},
    {0x0F12, 0x0158},
    {0x0F12, 0x02BA},
    {0x0F12, 0x01BA},
    {0x0F12, 0x0231},
    {0x0F12, 0x0252},
    {0x0F12, 0x0237},
    {0x0F12, 0x024C},
    {0x0F12, 0x020F},
    {0x0F12, 0x0279},
    {0x0F12, 0x0268},
    {0x0F12, 0x021A},
    {0x002A, 0x0C4C},
    {0x0F12, 0x0450},
    {0x002A, 0x0C58},
    {0x0F12, 0x059C},
    {0x002A, 0x0BF8},
    {0x0F12, 0x01AE},
    {0x002A, 0x0C28},
    {0x0F12, 0x0000},
    {0x002A, 0x0CAC},
    {0x0F12, 0x0050},
    {0x002A, 0x0C28},
    {0x0F12, 0x0000},

    {0x002A, 0x20BA},
    {0x0F12, 0x0006},
    
    {0x002A, 0x0D0E},
    {0x0F12, 0x00B8},
    {0x0F12, 0x00B2},
    {0x002A, 0x0CFE},
    {0x0F12, 0x0FAB},
    {0x0F12, 0x0FF5},
    {0x0F12, 0x10BB},
    {0x0F12, 0x1117},
    {0x0F12, 0x116D},
    {0x0F12, 0x11D5},
    {0x0F12, 0x00A9},
    {0x0F12, 0x00C0},
    {0x002A, 0x0CF8},
    {0x0F12, 0x02CC},
    {0x0F12, 0x031E},
    {0x0F12, 0x0359},

    {0x002A, 0x0CB0},
    {0x0F12, 0x0030},
    {0x0F12, 0x0040},
    {0x0F12, 0x0060},
    {0x0F12, 0x0040},
    {0x0F12, 0x0008},
    {0x0F12, 0x0008},

    {0x0F12, 0x0030},
    {0x0F12, 0x0040},
    {0x0F12, 0x0060},
    {0x0F12, 0x0040},
    {0x0F12, 0x0008},
    {0x0F12, 0x0008},

    {0x0F12, 0x0030},
    {0x0F12, 0x0040},
    {0x0F12, 0x0060},
    {0x0F12, 0x0040},
    {0x0F12, 0x0008},
    {0x0F12, 0x0008},

    {0x0F12, 0x0018},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0xFF80},
    {0x0F12, 0xFEC0},

    {0x0F12, 0x0018},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0xFF80},
    {0x0F12, 0xFEC0},

    {0x0F12, 0x0018},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0xFF80},
    {0x0F12, 0xFEC0},

    {0x002A, 0x0D30},
    {0x0F12, 0x0002},

    {0x002A, 0x3362},
    {0x0F12, 0x0001},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},

    {0x002A, 0x0C86},
    {0x0F12, 0x0005},
    {0x002A, 0x0C70},
    {0x0F12, 0xFF7B},
    {0x0F12, 0x00CE},
    {0x0F12, 0xFF23},
    {0x0F12, 0x010D},
    {0x0F12, 0xFEF3},
    {0x0F12, 0x012C},
    {0x0F12, 0xFED7},
    {0x0F12, 0x014E},
    {0x0F12, 0xFEBB},
    {0x0F12, 0x0162},
    {0x0F12, 0x1388},
    {0x002A, 0x0C8A},
    {0x0F12, 0x4ACB},
    {0x002A, 0x0C88},
    {0x0F12, 0x0A7C},

    {0x002A, 0x0CA0},
    {0x0F12, 0x0030},

    {0x002A, 0x0CA4},
    {0x0F12, 0x0030},
    {0x0F12, 0x0180},
    {0x0F12, 0x0002},

    {0x002A, 0x0D2A},
    {0x0F12, 0x058d}, //awb gain
    {0x0F12, 0x0400},
    {0x0F12, 0x070b},
//===================
// 10.Clock Setting
//===================
// Input Clock (Mclk)
    {0x002A, 0x012E},
    {0x0F12, 0x5DC0},
    {0x0F12, 0x0000},
    {0x002A, 0x0146},
    {0x0F12, 0x0000},
    {0x0F12, 0x0002},

    {0x002A, 0x014C},
    {0x0F12, 0x2CEC},
    {0x002A, 0x0152},
    {0x0F12, 0x59D8},
    {0x002A, 0x014E},
    {0x0F12, 0x59D8},
    {0x002A, 0x0154},
    {0x0F12, 0x2981},
    {0x002A, 0x015A},
    {0x0F12, 0x5208},
    {0x002A, 0x0156},
    {0x0F12, 0x53FC},

    {0x002A, 0x0164},
    {0x0F12, 0x0001},

    {0x0028, 0x7000},
    {0x002A, 0x03F4},
    {0x0F12, 0x0001},
    {0x0F12, 0x0001},
    {0x002A, 0x0408},
    {0x0F12, 0x067F},

    {0x002A, 0x0D40},
    {0x0F12, 0x003E},

    {0x002A, 0x0D46},
    {0x0F12, 0x000F},

    {0x002A, 0x0440},
    {0x0F12, 0x3410},
    {0x002A, 0x0444},
    {0x0F12, 0x6590},
    {0x002A, 0x0448},
    {0x0F12, 0xBB80},
    {0x002A, 0x044C},
    {0x0F12, 0x3880},
    {0x0F12, 0x0001},
    {0x002A, 0x0450},
    {0x0F12, 0x3410},
    {0x002A, 0x0454},
    {0x0F12, 0x6590},
    {0x002A, 0x0458},
    {0x0F12, 0xBB80},
    {0x002A, 0x045C},
    {0x0F12, 0x3880},
    {0x0F12, 0x0001},
    {0x0F12, 0x01B0},
    {0x0F12, 0x01B0},
    {0x0F12, 0x0280},
    {0x0F12, 0x0800},
    {0x0F12, 0x0100},
    {0x0F12, 0x3000},
    {0x002A, 0x042E},
    {0x0F12, 0x010E},
    {0x0F12, 0x00F5},
    {0x002A, 0x0DE0},
    {0x0F12, 0x0002},

    {0x002A, 0x0D38},
    {0x0F12, 0x0198},
    {0x002A, 0x0D3E},
    {0x0F12, 0x02A0},

    {0x002A, 0x0D4E},
    {0x0F12, 0x0000},
    {0x0F12, 0x0101},
    {0x0F12, 0x0101},
    {0x0F12, 0x0000},
    {0x0F12, 0x0201},
    {0x0F12, 0x0202},
    {0x0F12, 0x0202},
    {0x0F12, 0x0102},
    {0x0F12, 0x0201},
    {0x0F12, 0x0303},
    {0x0F12, 0x0303},
    {0x0F12, 0x0102},
    {0x0F12, 0x0201},
    {0x0F12, 0x0403},
    {0x0F12, 0x0304},
    {0x0F12, 0x0102},
    {0x0F12, 0x0201},
    {0x0F12, 0x0403},
    {0x0F12, 0x0304},
    {0x0F12, 0x0102},
    {0x0F12, 0x0201},
    {0x0F12, 0x0303},
    {0x0F12, 0x0303},
    {0x0F12, 0x0102},
    {0x0F12, 0x0201},
    {0x0F12, 0x0202},
    {0x0F12, 0x0202},
    {0x0F12, 0x0102},
    {0x0F12, 0x0000},
    {0x0F12, 0x0101},
    {0x0F12, 0x0101},
    {0x0F12, 0x0000},

    {0x002A, 0x33A4},
    {0x0F12, 0x01D0},
    {0x0F12, 0xFFA1},
    {0x0F12, 0xFFFA},
    {0x0F12, 0xFF6F},
    {0x0F12, 0x0140},
    {0x0F12, 0xFF49},
    {0x0F12, 0xFFC1},
    {0x0F12, 0x001F},
    {0x0F12, 0x01BD},
    {0x0F12, 0x013F},
    {0x0F12, 0x00E1},
    {0x0F12, 0xFF43},
    {0x0F12, 0x0191},
    {0x0F12, 0xFFC0},
    {0x0F12, 0x01B7},
    {0x0F12, 0xFF30},
    {0x0F12, 0x015F},
    {0x0F12, 0x0106},

    {0x0F12, 0x019F},
    {0x0F12, 0xFFA7},
    {0x0F12, 0x0026},
    {0x0F12, 0xFF63},
    {0x0F12, 0x01A4},
    {0x0F12, 0xFEEF},
    {0x0F12, 0xFF5B},
    {0x0F12, 0xFFE9},
    {0x0F12, 0x0259},
    {0x0F12, 0x0138},
    {0x0F12, 0x0138},
    {0x0F12, 0xFEF4},
    {0x0F12, 0x0135},
    {0x0F12, 0xFFF0},
    {0x0F12, 0x01E3},
    {0x0F12, 0xFED2},
    {0x0F12, 0x01BF},
    {0x0F12, 0x0102},

    {0x0F12, 0x01C3},
    {0x0F12, 0xFFA9},
    {0x0F12, 0xFFFE},
    {0x0F12, 0xFF5A},
    {0x0F12, 0x0145},
    {0x0F12, 0xFF5B},
    {0x0F12, 0xFFE2},
    {0x0F12, 0xFFF7},
    {0x0F12, 0x01BF},
    {0x0F12, 0x0147},
    {0x0F12, 0x00FC},
    {0x0F12, 0xFF21},
    {0x0F12, 0x0191},
    {0x0F12, 0xFFBC},
    {0x0F12, 0x01B6},
    {0x0F12, 0xFF40},
    {0x0F12, 0x0172},
    {0x0F12, 0x00E0},

    {0x0F12, 0x01F8},
    {0x0F12, 0xFF86},
    {0x0F12, 0xFFEE},
    {0x0F12, 0xFF5E},
    {0x0F12, 0x0143},
    {0x0F12, 0xFF56},
    {0x0F12, 0xFFFF},
    {0x0F12, 0xFFD2},
    {0x0F12, 0x01CA},
    {0x0F12, 0x01BC},
    {0x0F12, 0x0099},
    {0x0F12, 0xFF0E},
    {0x0F12, 0x01E1},
    {0x0F12, 0xFF81},
    {0x0F12, 0x01A6},
    {0x0F12, 0xFEFF},
    {0x0F12, 0x0186},
    {0x0F12, 0x010E},

    {0x0F12, 0x01BF},
    {0x0F12, 0xFFBF},
    {0x0F12, 0xFFFE},
    {0x0F12, 0xFF6D},
    {0x0F12, 0x01B4},
    {0x0F12, 0xFF66},
    {0x0F12, 0xFFCA},
    {0x0F12, 0xFFCE},
    {0x0F12, 0x017B},
    {0x0F12, 0x0136},
    {0x0F12, 0x0132},
    {0x0F12, 0xFF85},
    {0x0F12, 0x018B},
    {0x0F12, 0xFF73},
    {0x0F12, 0x0191},
    {0x0F12, 0xFF3F},
    {0x0F12, 0x015B},
    {0x0F12, 0x00D0},

    {0x0F12, 0x01D0},
    {0x0F12, 0xFF98},
    {0x0F12, 0x0015},
    {0x0F12, 0xFF3C},
    {0x0F12, 0x020D},
    {0x0F12, 0xFF3C},
    {0x0F12, 0xFF9F},
    {0x0F12, 0xFFAA},
    {0x0F12, 0x01C9},
    {0x0F12, 0x0182},
    {0x0F12, 0x0147},
    {0x0F12, 0xFF23},
    {0x0F12, 0x014D},
    {0x0F12, 0xFFB8},
    {0x0F12, 0x018B},
    {0x0F12, 0xFEA9},
    {0x0F12, 0x01CD},
    {0x0F12, 0x00F6},

    {0x002A, 0x3380},
    {0x0F12, 0x01AC},
    {0x0F12, 0xFFD7},
    {0x0F12, 0x0019},
    {0x0F12, 0xFF49},
    {0x0F12, 0x01D9},
    {0x0F12, 0xFF63},
    {0x0F12, 0xFFCA},
    {0x0F12, 0xFFCE},
    {0x0F12, 0x017B},
    {0x0F12, 0x0132},
    {0x0F12, 0x012E},
    {0x0F12, 0xFF8D},
    {0x0F12, 0x018B},
    {0x0F12, 0xFF73},
    {0x0F12, 0x0191},
    {0x0F12, 0xFF3F},
    {0x0F12, 0x015B},
    {0x0F12, 0x00D0},
    {0x002A, 0x0612},
    {0x0F12, 0x009D},
    {0x0F12, 0x00D5},
    {0x0F12, 0x0103},
    {0x0F12, 0x0128},
    {0x0F12, 0x0166},
    {0x0F12, 0x0193},

    {0x002A, 0x0538},
    {0x0F12, 0x0000},
    {0x0F12, 0x001F},
    {0x0F12, 0x0035},
    {0x0F12, 0x005A},
    {0x0F12, 0x0095},
    {0x0F12, 0x00E6},
    {0x0F12, 0x0121},
    {0x0F12, 0x0139},
    {0x0F12, 0x0150},
    {0x0F12, 0x0177},
    {0x0F12, 0x019A},
    {0x0F12, 0x01BB},
    {0x0F12, 0x01DC},
    {0x0F12, 0x0219},
    {0x0F12, 0x0251},
    {0x0F12, 0x02B3},
    {0x0F12, 0x030A},
    {0x0F12, 0x035F},
    {0x0F12, 0x03B1},
    {0x0F12, 0x03FF},
    {0x0F12, 0x0000},
    {0x0F12, 0x0001},
    {0x0F12, 0x0001},
    {0x0F12, 0x0002},
    {0x0F12, 0x0004},
    {0x0F12, 0x000A},
    {0x0F12, 0x0012},
    {0x0F12, 0x0016},
    {0x0F12, 0x001A},
    {0x0F12, 0x0024},
    {0x0F12, 0x0031},
    {0x0F12, 0x003E},
    {0x0F12, 0x004E},
    {0x0F12, 0x0075},
    {0x0F12, 0x00A8},
    {0x0F12, 0x0126},
    {0x0F12, 0x01BE},
    {0x0F12, 0x0272},
    {0x0F12, 0x0334},
    {0x0F12, 0x03FF},

    {0x002A, 0x0498},
    {0x0F12, 0x0000},
    {0x0F12, 0x0002},
    {0x0F12, 0x0007},
    {0x0F12, 0x001D},
    {0x0F12, 0x006E},
    {0x0F12, 0x00D3},
    {0x0F12, 0x0127},
    {0x0F12, 0x014C},
    {0x0F12, 0x016E},
    {0x0F12, 0x01A5},
    {0x0F12, 0x01D3},
    {0x0F12, 0x01FB},
    {0x0F12, 0x021F},
    {0x0F12, 0x0260},
    {0x0F12, 0x029A},
    {0x0F12, 0x02F7},
    {0x0F12, 0x034D},
    {0x0F12, 0x0395},
    {0x0F12, 0x03CE},
    {0x0F12, 0x03FF},
    {0x0F12, 0x0000},
    {0x0F12, 0x0004},
    {0x0F12, 0x000C},
    {0x0F12, 0x0024},
    {0x0F12, 0x006E},
    {0x0F12, 0x00D1},
    {0x0F12, 0x0119},
    {0x0F12, 0x0139},
    {0x0F12, 0x0157},
    {0x0F12, 0x018E},
    {0x0F12, 0x01C3},
    {0x0F12, 0x01F3},
    {0x0F12, 0x021F},
    {0x0F12, 0x0269},
    {0x0F12, 0x02A6},
    {0x0F12, 0x02FF},
    {0x0F12, 0x0351},
    {0x0F12, 0x0395},
    {0x0F12, 0x03CE},
    {0x0F12, 0x03FF},

    {0x002A, 0x3370},
    {0x0F12, 0x0000},

    {0x002A, 0x06D4},
    {0x0F12, 0x0032},
    {0x0F12, 0x0078},
    {0x0F12, 0x00C8},
    {0x0F12, 0x0190},
    {0x0F12, 0x028C},

    {0x002A, 0x0734},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0078},
    {0x0F12, 0x012C},
    {0x0F12, 0x03FF},
    {0x0F12, 0x0014},
    {0x0F12, 0x0064},
    {0x0F12, 0x000C},
    {0x0F12, 0x0010},
    {0x0F12, 0x01E6},
    {0x0F12, 0x0000},
    {0x0F12, 0x0070},
    {0x0F12, 0x01FF},
    {0x0F12, 0x0144},
    {0x0F12, 0x000F},
    {0x0F12, 0x000A},
    {0x0F12, 0x0073},
    {0x0F12, 0x0087},
    {0x0F12, 0x0014},
    {0x0F12, 0x000A},
    {0x0F12, 0x0023},
    {0x0F12, 0x001E},
    {0x0F12, 0x0014},
    {0x0F12, 0x000A},
    {0x0F12, 0x0023},
    {0x0F12, 0x0046},
    {0x0F12, 0x2B32},
    {0x0F12, 0x0601},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x00FF},
    {0x0F12, 0x07FF},
    {0x0F12, 0xFFFF},
    {0x0F12, 0x0000},
    {0x0F12, 0x050D},
    {0x0F12, 0x1E80},
    {0x0F12, 0x0000},
    {0x0F12, 0x1408},
    {0x0F12, 0x0214},
    {0x0F12, 0xFF01},
    {0x0F12, 0x180F},
    {0x0F12, 0x0001},
    {0x0F12, 0x0000},
    {0x0F12, 0x8003},
    {0x0F12, 0x0094},
    {0x0F12, 0x0580},
    {0x0F12, 0x0280},
    {0x0F12, 0x0308},
    {0x0F12, 0x3186},
    {0x0F12, 0x5260},
    {0x0F12, 0x0A02},
    {0x0F12, 0x080A},
    {0x0F12, 0x0500},
    {0x0F12, 0x032D},
    {0x0F12, 0x324E},
    {0x0F12, 0x001E},
    {0x0F12, 0x0200},
    {0x0F12, 0x0103},
    {0x0F12, 0x010C},
    {0x0F12, 0x9696},
    {0x0F12, 0x4646},
    {0x0F12, 0x0802},
    {0x0F12, 0x0802},
    {0x0F12, 0x0000},
    {0x0F12, 0x030F},
    {0x0F12, 0x3202},
    {0x0F12, 0x0F1E},
    {0x0F12, 0x020F},
    {0x0F12, 0x0103},
    {0x0F12, 0x010C},
    {0x0F12, 0x9696},
    {0x0F12, 0x4646},
    {0x0F12, 0x0802},
    {0x0F12, 0x0802},
    {0x0F12, 0x0000},
    {0x0F12, 0x030F},
    {0x0F12, 0x3202},
    {0x0F12, 0x0F1E},
    {0x0F12, 0x020F},
    {0x0F12, 0x0003},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x006A},
    {0x0F12, 0x012C},
    {0x0F12, 0x03FF},
    {0x0F12, 0x0014},
    {0x0F12, 0x0064},
    {0x0F12, 0x000C},
    {0x0F12, 0x0010},
    {0x0F12, 0x01E6},
    {0x0F12, 0x03FF},
    {0x0F12, 0x0070},
    {0x0F12, 0x007D},
    {0x0F12, 0x0064},
    {0x0F12, 0x0014},
    {0x0F12, 0x000A},
    {0x0F12, 0x0073},
    {0x0F12, 0x0087},
    {0x0F12, 0x0014},
    {0x0F12, 0x000A},
    {0x0F12, 0x0023},
    {0x0F12, 0x001E},
    {0x0F12, 0x0014},
    {0x0F12, 0x000A},
    {0x0F12, 0x0023},
    {0x0F12, 0x001E},
    {0x0F12, 0x2B32},
    {0x0F12, 0x0601},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x00FF},
    {0x0F12, 0x07FF},
    {0x0F12, 0xFFFF},
    {0x0F12, 0x0000},
    {0x0F12, 0x050D},
    {0x0F12, 0x1E80},
    {0x0F12, 0x0000},
    {0x0F12, 0x1408},
    {0x0F12, 0x0214},
    {0x0F12, 0xFF01},
    {0x0F12, 0x180F},
    {0x0F12, 0x0002},
    {0x0F12, 0x0000},
    {0x0F12, 0x8003},
    {0x0F12, 0x0080},
    {0x0F12, 0x0080},
    {0x0F12, 0x0280},
    {0x0F12, 0x0308},
    {0x0F12, 0x1E65},
    {0x0F12, 0x1A24},
    {0x0F12, 0x0A03},
    {0x0F12, 0x080A},
    {0x0F12, 0x0500},
    {0x0F12, 0x032D},
    {0x0F12, 0x324D},
    {0x0F12, 0x001E},
    {0x0F12, 0x0200},
    {0x0F12, 0x0103},
    {0x0F12, 0x010C},
    {0x0F12, 0x9696},
    {0x0F12, 0x2F34},
    {0x0F12, 0x0504},
    {0x0F12, 0x080F},
    {0x0F12, 0x0000},
    {0x0F12, 0x030F},
    {0x0F12, 0x3208},
    {0x0F12, 0x0F1E},
    {0x0F12, 0x020F},
    {0x0F12, 0x0103},
    {0x0F12, 0x010C},
    {0x0F12, 0x9696},
    {0x0F12, 0x1414},
    {0x0F12, 0x0504},
    {0x0F12, 0x080F},
    {0x0F12, 0x0000},
    {0x0F12, 0x030F},
    {0x0F12, 0x3208},
    {0x0F12, 0x0F1E},
    {0x0F12, 0x020F},
    {0x0F12, 0x0003},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0064},
    {0x0F12, 0x012C},
    {0x0F12, 0x03FF},
    {0x0F12, 0x0014},
    {0x0F12, 0x0064},
    {0x0F12, 0x000C},
    {0x0F12, 0x0010},
    {0x0F12, 0x01E6},
    {0x0F12, 0x03FF},
    {0x0F12, 0x0070},
    {0x0F12, 0x007D},
    {0x0F12, 0x0064},
    {0x0F12, 0x0096},
    {0x0F12, 0x0096},
    {0x0F12, 0x0073},
    {0x0F12, 0x0087},
    {0x0F12, 0x0014},
    {0x0F12, 0x0019},
    {0x0F12, 0x0023},
    {0x0F12, 0x001E},
    {0x0F12, 0x0014},
    {0x0F12, 0x0019},
    {0x0F12, 0x0023},
    {0x0F12, 0x001E},
    {0x0F12, 0x2B32},
    {0x0F12, 0x0601},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x00FF},
    {0x0F12, 0x07FF},
    {0x0F12, 0xFFFF},
    {0x0F12, 0x0000},
    {0x0F12, 0x050D},
    {0x0F12, 0x1E80},
    {0x0F12, 0x0000},
    {0x0F12, 0x0A08},
    {0x0F12, 0x0200},
    {0x0F12, 0xFF01},
    {0x0F12, 0x180F},
    {0x0F12, 0x0002},
    {0x0F12, 0x0000},
    {0x0F12, 0x8003},
    {0x0F12, 0x0080},
    {0x0F12, 0x0080},
    {0x0F12, 0x0280},
    {0x0F12, 0x0208},
    {0x0F12, 0x1E4B},
    {0x0F12, 0x1A24},
    {0x0F12, 0x0A05},
    {0x0F12, 0x080A},
    {0x0F12, 0x0500},
    {0x0F12, 0x032D},
    {0x0F12, 0x324D},
    {0x0F12, 0x001E},
    {0x0F12, 0x0200},
    {0x0F12, 0x0103},
    {0x0F12, 0x010C},
    {0x0F12, 0x9696},
    {0x0F12, 0x1E23},
    {0x0F12, 0x0505},
    {0x0F12, 0x080F},
    {0x0F12, 0x0000},
    {0x0F12, 0x030F},
    {0x0F12, 0x3208},
    {0x0F12, 0x0F1E},
    {0x0F12, 0x020F},
    {0x0F12, 0x0103},
    {0x0F12, 0x010C},
    {0x0F12, 0x9696},
    {0x0F12, 0x1E23},
    {0x0F12, 0x0505},
    {0x0F12, 0x080F},
    {0x0F12, 0x0000},
    {0x0F12, 0x030F},
    {0x0F12, 0x3208},
    {0x0F12, 0x0F1E},
    {0x0F12, 0x020F},
    {0x0F12, 0x0003},
    {0x0F12, 0x0000},
    {0x0F12, 0x0018},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0064},
    {0x0F12, 0x012C},
    {0x0F12, 0x03FF},
    {0x0F12, 0x0014},
    {0x0F12, 0x0064},
    {0x0F12, 0x000C},
    {0x0F12, 0x0010},
    {0x0F12, 0x01E6},
    {0x0F12, 0x0000},
    {0x0F12, 0x0070},
    {0x0F12, 0x007D},
    {0x0F12, 0x0064},
    {0x0F12, 0x0096},
    {0x0F12, 0x0096},
    {0x0F12, 0x0073},
    {0x0F12, 0x009F},
    {0x0F12, 0x0028},
    {0x0F12, 0x0028},
    {0x0F12, 0x0023},
    {0x0F12, 0x0037},
    {0x0F12, 0x0028},
    {0x0F12, 0x0028},
    {0x0F12, 0x0023},
    {0x0F12, 0x0037},
    {0x0F12, 0x2B32},
    {0x0F12, 0x0601},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x00FF},
    {0x0F12, 0x07A0},
    {0x0F12, 0xFFFF},
    {0x0F12, 0x0000},
    {0x0F12, 0x050D},
    {0x0F12, 0x1E80},
    {0x0F12, 0x0000},
    {0x0F12, 0x0A08},
    {0x0F12, 0x0200},
    {0x0F12, 0xFF01},
    {0x0F12, 0x180F},
    {0x0F12, 0x0001},
    {0x0F12, 0x0000},
    {0x0F12, 0x8003},
    {0x0F12, 0x0080},
    {0x0F12, 0x0080},
    {0x0F12, 0x0280},
    {0x0F12, 0x0108},
    {0x0F12, 0x1E32},
    {0x0F12, 0x1A24},
    {0x0F12, 0x0A05},
    {0x0F12, 0x080A},
    {0x0F12, 0x0000},
    {0x0F12, 0x0328},
    {0x0F12, 0x324C},
    {0x0F12, 0x001E},
    {0x0F12, 0x0200},
    {0x0F12, 0x0103},
    {0x0F12, 0x010C},
    {0x0F12, 0x9696},
    {0x0F12, 0x0F0F},
    {0x0F12, 0x0307},
    {0x0F12, 0x080F},
    {0x0F12, 0x0000},
    {0x0F12, 0x030F},
    {0x0F12, 0x3208},
    {0x0F12, 0x0F1E},
    {0x0F12, 0x020F},
    {0x0F12, 0x0103},
    {0x0F12, 0x010C},
    {0x0F12, 0x9696},
    {0x0F12, 0x0F0F},
    {0x0F12, 0x0307},
    {0x0F12, 0x080F},
    {0x0F12, 0x0000},
    {0x0F12, 0x030F},
    {0x0F12, 0x3208},
    {0x0F12, 0x0F1E},
    {0x0F12, 0x020F},
    {0x0F12, 0x0003},
    {0x0F12, 0x0000},
    {0x0F12, 0x0014},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0028},
    {0x0F12, 0x012C},
    {0x0F12, 0x03FF},
    {0x0F12, 0x0014},
    {0x0F12, 0x0064},
    {0x0F12, 0x000C},
    {0x0F12, 0x0010},
    {0x0F12, 0x01E6},
    {0x0F12, 0x0000},
    {0x0F12, 0x0070},
    {0x0F12, 0x0087},
    {0x0F12, 0x0073},
    {0x0F12, 0x0096},
    {0x0F12, 0x0096},
    {0x0F12, 0x0073},
    {0x0F12, 0x00B4},
    {0x0F12, 0x0028},
    {0x0F12, 0x0028},
    {0x0F12, 0x0023},
    {0x0F12, 0x0046},
    {0x0F12, 0x0028},
    {0x0F12, 0x0028},
    {0x0F12, 0x0023},
    {0x0F12, 0x0046},
    {0x0F12, 0x2B23},
    {0x0F12, 0x0601},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x0000},
    {0x0F12, 0x00FF},
    {0x0F12, 0x0B84},
    {0x0F12, 0xFFFF},
    {0x0F12, 0x0000},
    {0x0F12, 0x050D},
    {0x0F12, 0x1E80},
    {0x0F12, 0x0000},
    {0x0F12, 0x0A08},
    {0x0F12, 0x0200},
    {0x0F12, 0xFF01},
    {0x0F12, 0x180F},
    {0x0F12, 0x0001},
    {0x0F12, 0x0000},
    {0x0F12, 0x8003},
    {0x0F12, 0x0080},
    {0x0F12, 0x0080},
    {0x0F12, 0x0280},
    {0x0F12, 0x0108},
    {0x0F12, 0x1E1E},
    {0x0F12, 0x1419},
    {0x0F12, 0x0A0A},
    {0x0F12, 0x0800},
    {0x0F12, 0x0000},
    {0x0F12, 0x0328},
    {0x0F12, 0x324C},
    {0x0F12, 0x001E},
    {0x0F12, 0x0200},
    {0x0F12, 0x0103},
    {0x0F12, 0x010C},
    {0x0F12, 0x6464},
    {0x0F12, 0x0F0F},
    {0x0F12, 0x0307},
    {0x0F12, 0x080F},
    {0x0F12, 0x0000},
    {0x0F12, 0x030F},
    {0x0F12, 0x3208},
    {0x0F12, 0x0F1E},
    {0x0F12, 0x020F},
    {0x0F12, 0x0103},
    {0x0F12, 0x010C},
    {0x0F12, 0x6464},
    {0x0F12, 0x0F0F},
    {0x0F12, 0x0307},
    {0x0F12, 0x080F},
    {0x0F12, 0x0000},
    {0x0F12, 0x030F},
    {0x0F12, 0x3208},
    {0x0F12, 0x0F1E},
    {0x0F12, 0x020F},
    {0x0F12, 0x0003},
    {0x0F12, 0x7F5E},
    {0x0F12, 0xFEEE},
    {0x0F12, 0xD9B7},
    {0x0F12, 0x0472},
    {0x0F12, 0x0001},

// 20.Preview & Captu
//*******************
// Preview config[0] 
    {0x002A, 0x01BE},
    {0x0F12, 0x0500},
    {0x0F12, 0x03C0},
    {0x0F12, 0x0005},
    {0x002A, 0x01C8},
    {0x0F12, 0x0000},
    {0x002A, 0x01C4},
    {0x0F12, 0x0042},
    {0x002A, 0x01D4},
    {0x0F12, 0x0002},
    {0x002A, 0x01D2},
    {0x0F12, 0x0000},
    {0x002A, 0x01D8},
    {0x0F12, 0x03E8},
    {0x002A, 0x01D6},
    {0x0F12, 0x014D},
    {0x002A, 0x01E8},
    {0x0F12, 0x0003},
    {0x0F12, 0x0003},

    {0x002A, 0x02AE},
    {0x0F12, 0x0001},
    {0x002A, 0x02B0},
    {0x0F12, 0x0500},
    {0x0F12, 0x03C0},
    {0x0F12, 0x0005},
    {0x002A, 0x02BA},
    {0x0F12, 0x0000},
    {0x002A, 0x02B6},
    {0x0F12, 0x0042},
    {0x002A, 0x02C6},
    {0x0F12, 0x0002},
    {0x002A, 0x02C4},
    {0x0F12, 0x0000},//0x0002},
    {0x002A, 0x02CA},
    {0x0F12, 0x03e8},//0x014D},
    {0x002A, 0x02C8},
    {0x0F12, 0x014d},//0x0000},

    {0x002A, 0x01A8},
    {0x0F12, 0x0000},
    {0x002A, 0x01AA},
    {0x0F12, 0x0001},
    {0x002A, 0x019E},
    {0x0F12, 0x0001},
    {0x0F12, 0x0001},

    {0x0028, 0xD000},
    {0x002A, 0x1000},
};
static struct msm_camera_i2c_reg_conf s5k8aayx_recommend_settings_3[] = {
	{0x0F12, 0x0001},   // Set ho{0xt interrupt
    //mdelay(150);
};
static struct msm_camera_i2c_conf_array s5k8aayx_init_conf[] = {
    {&s5k8aayx_recommend_settings_0[0],
        ARRAY_SIZE(s5k8aayx_recommend_settings_0), 0, MSM_CAMERA_I2C_WORD_DATA},
    {&s5k8aayx_recommend_settings_1[0],
        ARRAY_SIZE(s5k8aayx_recommend_settings_1), 10, MSM_CAMERA_I2C_WORD_DATA},
    {&s5k8aayx_recommend_settings_2[0],
        ARRAY_SIZE(s5k8aayx_recommend_settings_2), 0, MSM_CAMERA_I2C_WORD_DATA},
    {&s5k8aayx_recommend_settings_3[0],
        ARRAY_SIZE(s5k8aayx_recommend_settings_3), 0, MSM_CAMERA_I2C_WORD_DATA},
};

static struct msm_camera_i2c_conf_array s5k8aayx_confs[] = {
	{&s5k8aayx_snap_settings[0],
        ARRAY_SIZE(s5k8aayx_snap_settings), 0, MSM_CAMERA_I2C_WORD_DATA},
	{&s5k8aayx_prev_settings[0],
        ARRAY_SIZE(s5k8aayx_prev_settings), 0, MSM_CAMERA_I2C_WORD_DATA},
};

static struct v4l2_subdev_info s5k8aayx_subdev_info[] = {
    {
            .code   = V4L2_MBUS_FMT_YUYV8_2X8,
            .colorspace = V4L2_COLORSPACE_JPEG,
            .fmt    = 1,
            .order    = 0,

    },
    /* more can be supported, to be added later */
};


static struct msm_sensor_output_info_t s5k8aayx_dimensions[] = {
	{ /* For SNAPSHOT */
		.x_output = 1280,/*1600*/  /*for 2Mp*/
		.y_output = 960,/*1200*/
		.line_length_pclk = 0x793,
		.frame_length_lines = 0x26A,
		.vt_pixel_clk = 0x892281,//0x2248A04,//,
		.op_pixel_clk = 133000000,//52000000,
		.binning_factor = 0,
	},
	{ /* For PREVIEW */
		.x_output = 1280,/*1600*/  /*for 2Mp*/
		.y_output = 960,/*1200*/
		.line_length_pclk = 0x793,
		.frame_length_lines = 0x26A,
		.vt_pixel_clk = 0x2248A04,//0x892281,//
		.op_pixel_clk = 133000000,//52000000,
		.binning_factor = 0,
	},
};

static struct msm_sensor_output_reg_addr_t s5k8aayx_reg_addr = {
	.x_output = 0x34C,
	.y_output = 0x34E,
	.line_length_pclk = 0x342,
	.frame_length_lines = 0x340,
};

static struct msm_sensor_id_info_t s5k8aayx_id_info = {
	.sensor_id_reg_addr = 0x0040,
	.sensor_id = 0x8aa,
};
#if 0
static struct msm_sensor_exp_gain_info_t s5k8aayx_exp_gain_info = {
        .coarse_int_time_addr = 0x3501,
        .global_gain_addr = 0x3508,
        .vert_offset = 6,
};
#endif
static enum msm_camera_vreg_name_t s5k8aayx_veg_seq[] = {
	CAM_VDIG,
    CAM_VIO,
//	CAM_VANA,
};
#if 0
static int32_t s5k8aayx_write_exp_gain(struct msm_sensor_ctrl_t *s_ctrl,
                uint16_t gain, uint32_t line)
{
	uint32_t fl_lines, offset;
	uint8_t int_time[3];
	fl_lines =
			(s_ctrl->curr_frame_length_lines * s_ctrl->fps_divider) / Q10;
	offset = s_ctrl->sensor_exp_gain_info->vert_offset;
	if (line > (fl_lines - offset))
			fl_lines = line + offset;
	
	s_ctrl->func_tbl->sensor_group_hold_on(s_ctrl);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_output_reg_addr->frame_length_lines, fl_lines,
			MSM_CAMERA_I2C_WORD_DATA);
	int_time[0] = line >> 12;
	int_time[1] = line >> 4;
	int_time[2] = line << 4;
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_exp_gain_info->coarse_int_time_addr-1,
			int_time[0], MSM_CAMERA_I2C_BYTE_DATA);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_exp_gain_info->coarse_int_time_addr,
			int_time[1], MSM_CAMERA_I2C_BYTE_DATA);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_exp_gain_info->coarse_int_time_addr+1,
			int_time[2], MSM_CAMERA_I2C_BYTE_DATA);
	msm_camera_i2c_write(s_ctrl->sensor_i2c_client,
			s_ctrl->sensor_exp_gain_info->global_gain_addr, gain,
			MSM_CAMERA_I2C_WORD_DATA);
	s_ctrl->func_tbl->sensor_group_hold_off(s_ctrl);
	return 0;
}
#endif
static const struct i2c_device_id s5k8aayx_i2c_id[] = {
	{SENSOR_NAME, (kernel_ulong_t)&s5k8aayx_s_ctrl},
	{ }
};

static struct i2c_driver s5k8aayx_i2c_driver = {
	.id_table = s5k8aayx_i2c_id,
	.probe  = msm_sensor_i2c_probe,
	.driver = {
		.name = SENSOR_NAME,
	},
};

static struct msm_camera_i2c_client s5k8aayx_sensor_i2c_client = {
	.addr_type = MSM_CAMERA_I2C_WORD_ADDR,
};


static const struct of_device_id s5k8aayx_dt_match[] = {
	{.compatible = "qcom,s5k8aayx", .data = &s5k8aayx_s_ctrl},
	{}
};

MODULE_DEVICE_TABLE(of, s5k8aayx_dt_match);

static struct platform_driver s5k8aayx_platform_driver = {
	.driver = {
		.name = "qcom,s5k8aayx",
		.owner = THIS_MODULE,
		.of_match_table = s5k8aayx_dt_match,
	},
};

static int32_t s5k8aayx_platform_probe(struct platform_device *pdev)
{
    int32_t rc = 0;
	const struct of_device_id *match;
	match = of_match_device(s5k8aayx_dt_match, &pdev->dev);
	rc = msm_sensor_platform_probe(pdev, match->data);
	return rc;
}

static int __init msm_sensor_init_module(void)
{
    int32_t rc = 0;
	rc = platform_driver_probe(&s5k8aayx_platform_driver,
	s5k8aayx_platform_probe);
	if (!rc)
		return rc;
	return i2c_add_driver(&s5k8aayx_i2c_driver);
}

static void __exit msm_sensor_exit_module(void)
{
	if (s5k8aayx_s_ctrl.pdev) {
		msm_sensor_free_sensor_data(&s5k8aayx_s_ctrl);
		platform_driver_unregister(&s5k8aayx_platform_driver);
	} else {
                i2c_del_driver(&s5k8aayx_i2c_driver);
	}
	return;
}

static struct v4l2_subdev_core_ops s5k8aayx_subdev_core_ops = {
#if EFFECT_ON
	.s_ctrl = msm_sensor_v4l2_s_ctrl,
	.queryctrl = msm_sensor_v4l2_query_ctrl,
#endif
	.ioctl = msm_sensor_subdev_ioctl,
	.s_power = msm_sensor_power,
};

static struct v4l2_subdev_video_ops s5k8aayx_subdev_video_ops = {
	.enum_mbus_fmt = msm_sensor_v4l2_enum_fmt,
};

static struct v4l2_subdev_ops s5k8aayx_subdev_ops = {
	.core = &s5k8aayx_subdev_core_ops,
	.video  = &s5k8aayx_subdev_video_ops,
};

static int32_t s5k8_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t ret = 0;

	ret = msm_camera_i2c_write(s_ctrl->sensor_i2c_client,0xFCFC,0x0000,MSM_CAMERA_I2C_WORD_DATA);
	if (ret < 0){
		printk("=====>%s %s SW reset[ERROR]\n", __func__, \
            s_ctrl->sensordata->sensor_name);
    }
    ret = msm_sensor_match_id(s_ctrl);
	if (!ret){
		printk("1st sub-cam\n");
    }
	return ret;
}
static int32_t s5k_sensor_write_res_settings(struct msm_sensor_ctrl_t *s_ctrl,
	uint16_t res)
{
	int32_t rc = 0;
	rc = msm_sensor_write_conf_array(
		s_ctrl->sensor_i2c_client,
		s_ctrl->msm_sensor_reg->mode_settings, res);
	return rc;
}
static int32_t s5k_sensor_setting(struct msm_sensor_ctrl_t *s_ctrl,
			int update_type, int res)
{
	int32_t rc = 0;
	if (update_type == MSM_SENSOR_REG_INIT) {
		s_ctrl->func_tbl->sensor_stop_stream(s_ctrl);
		msm_sensor_write_init_settings(s_ctrl);
	} else if (update_type == MSM_SENSOR_UPDATE_PERIODIC) {
		s5k_sensor_write_res_settings(s_ctrl, res);
		v4l2_subdev_notify(&s_ctrl->sensor_v4l2_subdev,
			NOTIFY_PCLK_CHANGE, &s_ctrl->msm_sensor_reg->
			output_settings[res].op_pixel_clk);
	}
	return rc;
}

static struct msm_sensor_fn_t s5k8aayx_func_tbl = {
	.sensor_match_id = s5k8_sensor_match_id,
	.sensor_start_stream = msm_sensor_start_stream,
	.sensor_stop_stream = msm_sensor_stop_stream,
	.sensor_set_fps = msm_sensor_set_fps,
#if 0
	.sensor_write_exp_gain = s5k8aayx_write_exp_gain,
	.sensor_write_snapshot_exp_gain = s5k8aayx_write_exp_gain,
#endif
	.sensor_setting = s5k_sensor_setting,
	.sensor_set_sensor_mode = msm_sensor_set_sensor_mode,
	.sensor_mode_init = msm_sensor_mode_init,
	.sensor_get_output_info = msm_sensor_get_output_info,
	.sensor_config = msm_sensor_config,
	.sensor_power_up = msm_sensor_power_up,
	.sensor_power_down = msm_sensor_power_down,
	//.sensor_adjust_frame_lines = msm_sensor_adjust_frame_lines2,
	.sensor_get_csi_params = msm_sensor_get_csi_params,
};

static struct msm_sensor_reg_t s5k8aayx_regs = {
	.default_data_type = MSM_CAMERA_I2C_WORD_DATA,
	.start_stream_conf = s5k8aayx_start_settings,
	.start_stream_conf_size = ARRAY_SIZE(s5k8aayx_start_settings),
	.stop_stream_conf = s5k8aayx_stop_settings,
	.stop_stream_conf_size = ARRAY_SIZE(s5k8aayx_stop_settings),
	.init_settings = &s5k8aayx_init_conf[0],
	.init_size = ARRAY_SIZE(s5k8aayx_init_conf),
	.mode_settings = &s5k8aayx_confs[0],
	.output_settings = &s5k8aayx_dimensions[0],
	.num_conf = ARRAY_SIZE(s5k8aayx_confs),
#if EFFECT_ON
    .no_effect_settings = &s5k8aayx_no_effect_confs[0],
#endif
};

static struct msm_sensor_ctrl_t s5k8aayx_s_ctrl = {
	.msm_sensor_reg = &s5k8aayx_regs,
#if EFFECT_ON
    .msm_sensor_v4l2_ctrl_info = s5k8aayx_v4l2_ctrl_info,
    .num_v4l2_ctrl = ARRAY_SIZE(s5k8aayx_v4l2_ctrl_info),
#endif
	.sensor_i2c_client = &s5k8aayx_sensor_i2c_client,
	.sensor_i2c_addr = 0x78,//0x60,
	.vreg_seq = s5k8aayx_veg_seq,
	.num_vreg_seq = ARRAY_SIZE(s5k8aayx_veg_seq),
	.sensor_output_reg_addr = &s5k8aayx_reg_addr,
	.sensor_id_info = &s5k8aayx_id_info,
	//.sensor_exp_gain_info = &s5k8aayx_exp_gain_info,
	.cam_mode = MSM_SENSOR_MODE_INVALID,
	.msm_sensor_mutex = &s5k8aayx_mut,
	.sensor_i2c_driver = &s5k8aayx_i2c_driver,
	.sensor_v4l2_subdev_info = s5k8aayx_subdev_info,
	.sensor_v4l2_subdev_info_size = ARRAY_SIZE(s5k8aayx_subdev_info),
	.sensor_v4l2_subdev_ops = &s5k8aayx_subdev_ops,
	.func_tbl = &s5k8aayx_func_tbl,
	.clk_rate = MSM_SENSOR_MCLK_24HZ,
};

module_init(msm_sensor_init_module);
module_exit(msm_sensor_exit_module);
MODULE_DESCRIPTION("Sumsang 1.3MP YUV sensor driver");
MODULE_LICENSE("GPL v2");
