	...
	uint16_t res)
{
	...
	if (rc < 0)
		return rc;
/*[PLATFORM]-Mod-BEGIN by TCTNB.ZL, 2012/12/18, avoid error for gc0313 re-writing*/
	if (s_ctrl->sensor_i2c_addr == 0x42)
		return rc;
	else{
...
/*[PLATFORM]-Add-BEGIN by TCTNB.ZL, 2012/11/30, add this config for both viper and spyder*/
	gpio_request(CAM_AVDD_EN, "camera_avdd_en");
	gpio_tlmm_config(GPIO_CFG(CAM_AVDD_EN, 0,
...
/*[PLATFORM]-Add-BEGIN by TCTNB.HJ, 2013/03/26, BUG[diablolTE][403123] to optimize the power timing*/
	/*[PLATFORM]-Add-BEGIN by TCTNB.ZL, 2012/11/30, add this config for both viper and spyder*/
...
int32_t msm_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	gpio_free(CAM_AVDD_EN);//[PLATFORM]-Add by TCTNB.ZL,2012/11/27,add this for V00 Viper, will change in next hardware version
	kfree(s_ctrl->reg_ptr);
	s_ctrl->curr_res = MSM_SENSOR_INVALID_RES;
	return 0;
}
