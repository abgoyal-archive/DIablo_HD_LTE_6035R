				...
				uint32_t id, struct msm_ctrl_cmd *out)
{
	...
	struct msm_isp_event_ctrl *isp_event;
	void *ctrlcmd_data;
/*[PLATFORM]-Mod-BEGIN by TCTNB.HJ, 2012/12/21, for spyderLTE front camera, refer to bug 371654*/
#if (defined CONFIG_S5K8AAYX_TCT || defined CONFIG_S5K8AAYX_TCT_SECOND)
	char *sensor_device_name;
	sensor_device_name = server_dev->sensor_device[out->vnode_id]->devnode->name;
#endif
...
#if (defined CONFIG_S5K8AAYX_TCT || defined CONFIG_S5K8AAYX_TCT_SECOND) 
/*[PLATFORM]-Mod-BEGIN by TCTNB.HJ, 2012/12/21, for (spyderLTE) front camera, refer to bug 371654*/
		//printk("=====> sensor's name:[%s]\n", sensor_device_name);
		if (!strncmp("s5k8aayx 4-0078", sensor_device_name, strlen("s5k8aayx 4-0078"))||
            !strncmp("s5k8aayx_second 4-0020", sensor_device_name, strlen("s5k8aayx_second 4-0020"))){
			//printk("=====> for (spyderLTE)&s5k8 camera\n");
            rc = wait_event_interruptible_timeout(queue->wait,
					!list_empty_careful(&queue->list),
					msecs_to_jiffies(out->timeout_ms) + msecs_to_jiffies(5*1000));
		}
        else{
            //printk("=====> for (spyderLTE) other's camera\n");
            rc = wait_event_interruptible_timeout(queue->wait,
                !list_empty_careful(&queue->list),
...
/*[PLATFORM]-Mod-END by TCTNB.HJ */
#else
		//printk("=====> for (non-spyderLTE)'s camera\n");
		rc = wait_event_interruptible_timeout(queue->wait,
			!list_empty_careful(&queue->list),
			msecs_to_jiffies(out->timeout_ms));
#endif
	...
	if (list_empty_careful(&queue->list)) {
		if (!rc) {
/*[PLATFORM]-Mod-BEGIN by TCTNB.HJ, 2012/12/21, mod log*/
#ifdef CONFIG_TCT_8X30_COMMON
			printk("=====> %s TIMEOUT && rc=0\n", __func__);
#endif
/*[PLATFORM]-Mod-END by TCTNB.HJ */
            rc = -ETIMEDOUT;
		msm_drain_eventq(
