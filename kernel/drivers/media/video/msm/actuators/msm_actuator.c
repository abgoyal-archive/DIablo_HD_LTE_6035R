 ...
 * --------------------------------------------------------------------------------------
 * 13/06/15      Hu Jin       Camera Consumption fly mode 26mA.
 * 13/07/16      Hu Jin       300 uA.
 * 13/07/18      Hu Jin       crash
 * --------------------------------------------------------------------------------------
 */
	...
	int16_t next_lens_position, uint32_t hw_params, uint16_t delay)
{
	...
	CDBG("%s: IN\n", __func__);
/*[PLATFORM]-Add-BEGIN by TCTNB.HJ, 2013/07/18, BUG[486245]*/
	if (!a_ctrl->i2c_reg_tbl){
		printk("a_ctrl->i2c_reg_tbl NULL\n");
		return 0;
	}
...
/*[PLATFORM]-Add-BEGIN by TCTNB.HJ, 2013/06/15, BUG[402875]*/
#ifdef CONFIG_MACH_MSM8X30_DIABLOLTE
static int32_t msm_actuator_xsd(struct msm_actuator_ctrl_t *a_ctrl)
{
    int32_t rc = 0;
    a_ctrl->i2c_tbl_index = 0;
    rc = a_ctrl->func_tbl->actuator_parse_i2c_params(a_ctrl,
    ...
    rc = a_ctrl->func_tbl->actuator_parse_i2c_params(a_ctrl,
        a_ctrl->initial_code, 0, 0);
    a_ctrl->i2c_reg_tbl->reg_addr |= 0x80;
#if 0
    rc = msm_camera_i2c_write_table_w_microdelay(
        &a_ctrl->i2c_client, a_ctrl->i2c_reg_tbl,
        a_ctrl->i2c_tbl_index, a_ctrl->i2c_data_type);
#else
	rc = msm_camera_i2c_write(&a_ctrl->i2c_client, a_ctrl->i2c_reg_tbl->reg_addr,
           0x8000, MSM_CAMERA_I2C_WORD_ADDR);
#endif
    if (rc < 0) {
        pr_err("%s: i2c write error:%d\n",
            __func__, rc);
...
{
	int32_t rc = 0;
/*[PLATFORM]-Add-BEGIN by TCTNB.HJ, 2013/06/15, BUG[453784]*/
#ifdef CONFIG_MACH_MSM8X30_DIABLOLTE
	if (!a_ctrl->vcm_enable) {
		rc = msm_actuator_xsd(a_ctrl);
	}
#endif
/*[PLATFORM]-Add-END by TCTNB.HJ*/
