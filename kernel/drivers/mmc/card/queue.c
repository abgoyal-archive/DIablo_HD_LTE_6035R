...
//[BUGFIX]-Mod by TCTNB.ZL,06/24/2013,PR461273
	int retries;
	...
	if (!sg) {
		...
		while (retries) {
			...
			if (!sg){
				*err = -ENOMEM;
				printk("%s--------re-alloc--------size=%d,retries=%d\n",__func__,sizeof(struct scatterlist)*sg_len,retries);
				retries--;
				continue;
			}
			else {
