...
/* ----------|----------------------|----------------------|----------------- */
/* 07/04/2013|     yujun.chang      |        482369        |can not hear sou- */
/*           |                      |                      |nd form BT Heads- */
/*           |                      |                      |et                */
/* ----------|----------------------|----------------------|----------------- */
/*07/11/2013 |     yujun.chang      |        460673        |can not hear sound*/
/******************************************************************************/
	...
	while (frames_left >= codesize) {
		...
		if ((data->frame_count == 15) || (data->count + written >= data->link_mtu) ||
			...
			VDBG("sending packet %d, count %d, link_mtu %u",
					...
					data->link_mtu);
            //[BUGFIX]-ADD-BIN-BY-TCT.NB.yujun.chang,460673,07/11/3013,not hear sound form BT
            if(data->state == A2DP_STATE_STARTED){
			    err = avdtp_write(data);
            }
            //[BUGFIX]-ADD-END-BY-TCTNB.yujun.chang
			if (err < 0)
				return err;
		}
